...
//!
//! The mapping itself lives in `gfs_search::exit_code` and is called from here,
//! so `gfs search` and `gfs rg` cannot drift apart and neither can invent its
//! own interpretation.
//!
...
use gfs_mount::search::SearchReport;
use gfs_search::query::{ExecutionStatus, SearchOutcome};
...
pub fn print_diagnostics(report: &SearchReport, err: &mut impl Write) -> std::io::Result<()> {
  match &report.outcome {
    SearchOutcome::FailedBeforeCompletion(reason) => {
      writeln!(
        err,
        "gfs search: the search did not complete: {reason}\n\
    ...
    SearchOutcome::Completed(result) => {
      ...
      if completion.execution_status == ExecutionStatus::Truncated {
        ...
        writeln!(
          err,
          "gfs search: TRUNCATED ({reason}). This is not an exhaustive answer."
        )?;
      ...
      if !gaps.is_empty() {
        writeln!(
          err,
          "gfs search: {} of {} paths in scope were not searched: {}",
          gaps
            .iter()
...
pub fn exit_code(report: &SearchReport, require_exhaustive: bool) -> i32 {
  gfs_search::exit_code(&report.outcome, require_exhaustive)
}
...
mod tests {
  use super::*;
  use gfs_search::query::{Completion, Coverage, Match, SearchResult, TruncationReason};
