...
use gfs_mount::control::{LogEntry, Request, Response};
  ...
  while i < argv.len() {
    ...
    match arg {
      ...
      "-p" | "--patch" | "-S" | "--follow" | "--stat" | "--graph" => bail!(
        "gfs log does not support `{arg}`: it needs a tree or blob per commit, \
         which is the unbounded download ADR 0005 exists to avoid. Use \
         `gfs rg` to search content, or clone through the gateway for full \
         history tooling."
      ),
      other => bail!(
...
fn print_help() {
  println!(
    "gfs log: commit history for a GFS workspace, answered by the server.\n\
     \n\
     USAGE:\n    \
       gfs log [-n <count>] [--oneline] [--skip <n>]\n    \
       gfs log -10 --format='%h %s'\n\
     \n\
     OPTIONS:\n    \
