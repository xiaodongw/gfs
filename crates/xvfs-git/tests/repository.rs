...
use xvfs_git::{GitRepository, Libgit2Repository};
use xvfs_types::error::ErrorCode;
use xvfs_types::{limits, mode, BytePath, EntryKind, HashAlgorithm, ObjectId, RevisionSelector};

const TREE_CACHE_BYTES: usize = 4 * 1024 * 1024;

fn open(name: &str) -> Libgit2Repository {
  Libgit2Repository::open(xvfs_test::bare(name), 4, TREE_CACHE_BYTES)
...
fn every_fixture_matches_its_declared_openability() {
  ...
  for f in xvfs_test::FIXTURES {
    let result = Libgit2Repository::open(xvfs_test::bare(f.name), 2, TREE_CACHE_BYTES);
    assert_eq!(
      result.is_ok(),
      f.openable,
...
fn resolves_branches_lightweight_tags_annotated_tags_and_oids() {
  let repo = open("basic");
  let expected = xvfs_test::git(&xvfs_test::bare("basic"), &["rev-parse", "main"])
    .unwrap()
    .trim()
    .to_owned();
  ...
  let expected_tree = xvfs_test::git(&xvfs_test::bare("basic"), &["rev-parse", "main^{tree}"])
    .unwrap()
    .trim()
    .to_owned();
...
fn resolution_refuses_the_reserved_namespace_even_when_the_ref_exists() {
  ...
  let (_tmp, path) = xvfs_test::scratch_clone("basic").unwrap();
  let repo = Libgit2Repository::open(&path, 2, TREE_CACHE_BYTES).unwrap();
  let commit = head(&repo);
  let anchor = xvfs_types::revision::lease_anchor_ref("m-test");
  ...
  assert_eq!(
    RevisionSelector::parse("xvfs/mounts/m-test", repo.algorithm())
      .unwrap_err()
      .code,
    ErrorCode::ReservedNamespace
  );
  ...
  let names: Vec<String> = repo
    ...
    .collect();
  assert!(
    !names.iter().any(|n| n.starts_with("refs/xvfs/")),
...
fn entry_lookup_matches_git_ls_tree_for_every_mode() {
  let repo = open("modes");
  let bare = xvfs_test::bare("modes");
  let commit = head(&repo);

  // The oracle: stock Git's own view of the tree.
  let listing = xvfs_test::git(&bare, &["ls-tree", "-r", "-t", "HEAD"]).unwrap();
  let mut checked = 0;
...
fn paging_a_large_directory_returns_every_entry_exactly_once() {
  ...
  let commit = head(&repo);
  let bare = xvfs_test::bare("bigdir");

  // The oracle: how many entries Git says are in that directory.
  let expected: usize = xvfs_test::git(&bare, &["ls-tree", "HEAD:many"])
    .unwrap()
    .lines()
    .count();
...
fn blob_bytes_match_git_cat_file_exactly_including_crlf_and_no_final_newline() {
  ...
  let bare = xvfs_test::bare("content");
  let commit = head(&repo);
...
fn an_anchor_for_a_nonexistent_commit_is_refused() {
  ...
  let anchor = xvfs_types::revision::lease_anchor_ref("m-dangling");
  assert_eq!(
...
fn visibility_tracks_reachability_from_visible_refs_only() {
  ...
  xvfs_test::git(&path, &["update-ref", "refs/heads/main", &first.to_hex()]).unwrap();
  xvfs_test::git(&path, &["update-ref", "-d", "refs/heads/feature"]).unwrap();
  xvfs_test::git(&path, &["tag", "-d", "v2.0"]).unwrap();
  xvfs_test::git(&path, &["tag", "-d", "tree-tag"]).unwrap();
  ...
  let anchor = xvfs_types::revision::lease_anchor_ref("m-vis");
...
fn a_leased_commit_survives_a_force_push_a_branch_deletion_and_a_full_gc() {
  ...
  let anchor = xvfs_types::revision::lease_anchor_ref("m-gc");
  repo.create_lease_anchor(&anchor, &leased).unwrap();

  // Everything that could make the commit unreachable.
  xvfs_test::git(&path, &["update-ref", "refs/heads/main", &older.to_hex()]).unwrap();
  xvfs_test::git(&path, &["update-ref", "-d", "refs/heads/feature"]).unwrap();
  xvfs_test::git(&path, &["tag", "-d", "v2.0"]).unwrap();
  xvfs_test::git(&path, &["tag", "-d", "tree-tag"]).unwrap();
...
fn upstream_fetch_and_prune_cannot_remove_a_lease_anchor() {
  // ADR 0006: `refs/xvfs/` is excluded from every upstream fetch and prune
  ...
  let (_upstream_tmp, upstream) = xvfs_test::scratch_clone("basic").unwrap();
  let (_tmp, path) = xvfs_test::scratch_clone("basic").unwrap();
  let repo = Libgit2Repository::open(&path, 2, TREE_CACHE_BYTES).unwrap();

  let leased = head(&repo);
  let anchor = xvfs_types::revision::lease_anchor_ref("m-prune");
  repo.create_lease_anchor(&anchor, &leased).unwrap();
  ...
  xvfs_test::git(&upstream, &["tag", "-d", "v2.0"]).unwrap();
  ...
  xvfs_test::git(
    ...
    &[
      ...
      "--prune-tags",
      &upstream_url,
      // Explicit refspecs, never `--mirror`: the namespaces XVFS mirrors are
...
fn lease_anchors_are_absent_from_stock_git_advertisements() {
  ...
  let repo = Libgit2Repository::open(&path, 2, TREE_CACHE_BYTES).unwrap();
  let commit = head(&repo);
  let anchor = xvfs_types::revision::lease_anchor_ref("m-adv");
  repo.create_lease_anchor(&anchor, &commit).unwrap();
  ...
  let bare_advertised =
    xvfs_test::git(&path, &["ls-remote", "--refs", path.to_str().unwrap()]).unwrap();
  assert!(
    bare_advertised.contains("refs/xvfs/"),
