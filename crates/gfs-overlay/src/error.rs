...
//!
//! `GfsError`'s codes are a *service* vocabulary: they describe what a remote
...
pub enum Condition {
  ...
  ///
  /// Used for the object types GFS does not model: FIFOs, block and character
...
impl From<GfsError> for OverlayError {
  fn from(e: GfsError) -> Self {
    match e.code {
      ErrorCode::InvalidArgument => OverlayError::new(
        Condition::Invalid,
...
mod tests {
  ...
  fn a_service_error_becomes_eio_with_its_message_intact() {
    let e: OverlayError = GfsError::not_found("no such blob").into();
