...
//! `gfs-server` reaches its catalog.
//!
...
impl Overlay {
  ...
  fn next_time(&self, inner: &mut Inner) -> Timestamp {
    let t = gfs_types::time::overlay_time(Timestamp::now(), self.snapshot_time, Some(inner.clock));
    inner.clock = t;
    t
  }
...
impl OverlayError {
  ...
  fn not_permitted_kind(kind: gfs_types::EntryKind) -> OverlayError {
    OverlayError::new(
