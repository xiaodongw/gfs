//! `xvfs-rg`: the `rg` substitute an agent can safely be given.
//!
//! PLAN.md M4.5: "Implement `xvfs-rg` for the selected safe flag subset and fail
//! closed on unsupported flags unless explicit hydration is requested."
//!
//! # Why a substitute is needed at all
//!
//! Real `rg` inside an XVFS mount walks every directory and reads every file.
...
//! property the project exists for. `xvfs-rg` answers the same question from the
...
//! how `xvfs-rg -w authorize` silently returns non-word-boundary matches and an
//! agent acts on a wrong answer. `--hydrate` is the documented escape hatch: it
//! says "run real `rg` over the mount, and accept the cost", so a flag XVFS
...
use xvfs_fuse::search::SearchRequest;

/// The flags `xvfs-rg` implements, and how they map.
///
...
fn main() -> ExitCode {
  match run() {
    ...
    Err(e) => {
      eprintln!("xvfs-rg: {e:#}");
      // 2 is the contract's "the search did not produce an answer" code, and an
      // argument error is exactly that: no search ran.
      ExitCode::from(2)
    }
  }
}
...
fn run() -> Result<i32> {
  ...
  let scope = match args.paths.len() {
    ...
    _ => bail!(
      "xvfs-rg takes at most one path argument; got {}. \
...
fn hydrate(workspace_root: &std::path::Path, argv: &[String]) -> Result<i32> {
  ...
  eprintln!(
    "xvfs-rg: --hydrate: running real rg over {}. This walks the mount and \
...
fn unsupported(flag: &str) -> anyhow::Error {
  anyhow::anyhow!(
    "unsupported flag {flag}.\n\
     `xvfs-rg` implements a deliberately small subset of ripgrep and refuses \
...
fn print_help() {
  println!("xvfs-rg -- search an XVFS workspace without hydrating it\n");
  println!("USAGE:\n  xvfs-rg [OPTIONS] PATTERN [PATH]\n");
  println!("OPTIONS:");
