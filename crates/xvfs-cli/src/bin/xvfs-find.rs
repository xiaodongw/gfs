//! `xvfs-find`: the `find -name` substitute an agent can safely be given.
//!
//! # Why a substitute is needed at all
//!
//! Real `find` inside an XVFS mount walks every directory, which on the M0.1
...
//!
//! `xvfs-find` asks the server for the pinned commit's matching paths and merges
...
//! # Exit codes
//!
//! ripgrep's, and the same ones `xvfs-rg` uses, because an agent should not have
...
use xvfs_fuse::find::FindRequest;

fn main() -> ExitCode {
  match run() {
    Ok(code) => ExitCode::from(code as u8),
    Err(e) => {
      eprintln!("xvfs-find: {e:#}");
      // No search ran, so there is no answer. Never 1: that would say the file
      // does not exist.
      ExitCode::from(2)
    }
  }
...
fn run() -> Result<i32> {
  ...
  if args.json {
    ...
    if report.truncated {
      eprintln!(
        "xvfs-find: truncated at {} paths; narrow the glob or raise --max-results",
        report.paths.len()
      );
    }
  }
...
fn parse(argv: &[String]) -> Result<Args> {
  ...
  while i < argv.len() {
    ...
    match arg {
      ...
      // learned the limit flag from `xvfs-rg` does not get a refusal here.
      ...
      // A bare word is the name pattern, so `xvfs-find '*.py'` works the way an
      ...
      other => bail!(
        "xvfs-find does not support `{other}`. Supported: -g/--glob, --exclude, \
...
fn print_help() {
  println!(
    "xvfs-find: filename search over an XVFS workspace, answered by the server.\n\
     \n\
     USAGE:\n    \
       xvfs-find <glob> [scope]\n    \
       xvfs-find -g '*.py' -g '*.pyi' --exclude '*/tests/*'\n\
     \n\
     OPTIONS:\n    \
