...
//!
//! `gfs-server`'s catalog runs `synchronous = FULL` because a lease record that
...
pub(crate) fn db_error(e: rusqlite::Error) -> GfsError {
  GfsError::new(ErrorCode::Internal, format!("search index error: {e}"))
}
...
mod tests {
  ...
  fn a_newer_schema_is_refused_rather_than_read() {
    let dir = std::env::temp_dir().join(format!("gfs-search-schema-{}", std::process::id()));
