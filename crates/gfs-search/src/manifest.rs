...
//!
//! Symlinks, gitlinks, and unsupported modes are absent. `gfs_git::WalkEntry`
//! carries the reasoning: `rg` does not follow symlinks by default, so including
//! them would make GFS return matches `rg` does not.
//!
...
// Padded to 8 bytes rather than shortened when `XVFSMAN\0` became `GFSMAN`: the
// width is the format's, not the name's, and a reader that trusted the old
// length would mis-frame every field after it. The rename is an incompatible
// change to the bytes on disk, so a manifest written before it is rejected by
// the magic check rather than being misread -- which is why the check exists.
const MAGIC: &[u8; 8] = b"GFSMAN\x00\x00";
...
impl Manifest {
  ...
  pub fn decode(bytes: &[u8]) -> Result<Manifest, GfsError> {
...
///
/// Distinct from `gfs_git::TreeDelta`, which carries an object ID: interning
...
impl<'a> Cursor<'a> {
  fn take(&mut self, n: usize) -> Result<&'a [u8], GfsError> {
    let end = self
      .at
      .checked_add(n)
  ...
  fn varint(&mut self) -> Result<u64, GfsError> {
    let mut value = 0u64;
    let mut shift = 0;
    loop {
      let byte = *self
...
fn corrupt(message: impl Into<String>) -> GfsError {
  GfsError::new(ErrorCode::Internal, message)
}
...
mod tests {
  ...
  fn a_first_parent_delta_costs_the_diff_rather_than_the_tree() {
    ...
    let child = parent.apply(
      ...
      &[
        ...
        ManifestDelta::Upserted {
          ...
          mode: gfs_types::mode::EXECUTABLE,
          key: 9,
        },
        ManifestDelta::Upserted {
    ...
    assert_eq!(child.paths()[0].mode, gfs_types::mode::EXECUTABLE);
