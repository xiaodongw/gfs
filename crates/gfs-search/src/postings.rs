...
impl PostingStore {
  ...
  pub fn merge(&self, batch: &PostingBatch) -> Result<(), GfsError> {
    if batch.is_empty() {
      return Ok(());
    }
  ...
  pub fn posting(&self, t: u32) -> Result<Option<RoaringBitmap>, GfsError> {
    let repository = self.repository.as_str().to_owned();
  ...
  /// variants.
  fn posting_folded(&self, t: u32, case_insensitive: bool) -> Result<RoaringBitmap, GfsError> {
    if !case_insensitive {
  ...
  pub fn stored_bytes(&self) -> Result<u64, GfsError> {
    let repository = self.repository.as_str().to_owned();
  ...
  pub fn trigram_count(&self) -> Result<u64, GfsError> {
    let repository = self.repository.as_str().to_owned();
    self.store.with_conn(|conn| {
      conn
        .query_row(
...
fn decode_bitmap(bytes: &[u8]) -> Result<RoaringBitmap, GfsError> {
  RoaringBitmap::deserialize_from(bytes)
    .map_err(|e| GfsError::internal(format!("a stored posting list is unreadable: {e}")))
}
