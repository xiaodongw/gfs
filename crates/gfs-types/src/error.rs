//! Structured error codes shared by every GFS surface.
//!
...
//!
//! Every `GfsError` message is constructed to be safe for a client *and* for a
...
pub enum ErrorCode {
  ...
  NotIndexable,
  /// The repository's on-disk format is one GFS cannot serve. ADR 0001 rejects
  /// `reftable`, SHA-256, and unrecognized `extensions.*` at ingest rather than
  /// serving a partial view.
  UnsupportedRepositoryFormat,
  /// The caller named `refs/gfs/`, which is an internal reachability namespace
...
impl ErrorCode {
  ...
  ///
  /// Returned as a number so `gfs-types` does not depend on `tonic`; the
  /// server converts.
...
impl GfsError {
  ...
  pub fn not_found(message: impl Into<String>) -> Self {
    GfsError::new(ErrorCode::NotFound, message)
  }

  pub fn invalid(message: impl Into<String>) -> Self {
    GfsError::new(ErrorCode::InvalidArgument, message)
  }
...
impl fmt::Display for GfsError {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(f, "{}: {}", self.code, self.message)
  }
}

impl fmt::Debug for GfsError {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(f, "{self}")
  }
}
...
impl From<crate::oid::OidError> for GfsError {
  fn from(e: crate::oid::OidError) -> Self {
    GfsError::new(ErrorCode::InvalidArgument, format!("object id: {e}"))
  }
}
...
mod tests {
  ...
  fn json_representation_uses_the_wire_name() {
    let e = GfsError::new(ErrorCode::SnapshotBuilding, "manifest is building");
    let json = serde_json::to_string(&e).unwrap();
    assert_eq!(
      json,
      r#"{"code":"SNAPSHOT_BUILDING","message":"manifest is building"}"#
    );
    let back: GfsError = serde_json::from_str(&json).unwrap();
    assert_eq!(back, e);
  }
  ...
  fn a_masked_denial_is_indistinguishable_from_an_absence() {
    let denied = GfsError::masked_denial("repository not found");
    let absent = GfsError::not_found("repository not found");
