...
impl RepositoryId {
  ...
  pub fn parse(s: &str) -> Result<Self, GfsError> {
    ...
    if s.starts_with('.') {
      return Err(GfsError::invalid("repository id may not start with `.`"));
    }
    if s.contains("..") {
      return Err(GfsError::invalid("repository id may not contain `..`"));
    }
    if !s
      .bytes()
...
impl MountId {
  pub fn parse(s: &str) -> Result<Self, GfsError> {
    ...
    // into `refs/gfs/mounts/{id}`, so a `.` would risk a `..` or a `.lock`
...
mod tests {
  ...
  fn mount_id_cannot_produce_an_invalid_ref_name() {
    // A mount id is interpolated into `refs/gfs/mounts/{id}`, so anything that
