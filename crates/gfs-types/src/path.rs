...
//! are implemented here rather than pulled from a crate so that `gfs-types`
...
impl BytePath {
  ...
  pub fn validate(&self) -> Result<(), GfsError> {
  ...
  pub fn from_b64url(s: &str) -> Result<Self, GfsError> {
...
pub fn b64url_decode(s: &str) -> Result<Vec<u8>, GfsError> {
  ...
  let bad = || GfsError::new(ErrorCode::InvalidArgument, "value is not base64url");
