//! Correctness oracles: an independent answer to compare GFS against.
//!
...
pub fn materialize_raw(repo: &Path, revision: &str, destination: &Path) -> Result<()> {
  ...
  for record in listing.split(|b| *b == 0).filter(|r| !r.is_empty()) {
    ...
    match *mode {
      ...
      // A submodule. GFS presents it as an empty read-only directory, and the
    ...
    .flatten()
  {
    ...
    if kind.is_symlink() {
      ...
      out.insert(
        child,
        EntrySnapshot::File {
          // Only the executable bit: GFS reports a read-only base, so comparing
...
mod tests {
  ...
  fn the_raw_materializer_does_not_convert_line_endings() {
    ...
    // bytes are LF. GFS serves the raw bytes.
