...
//! # Caching
//!
//! Fixtures are built once into `target/gfs-fixtures/<version>/` and reused. The
...
fn build_modes(dir: &Path) -> Result<()> {
  ...
  // 160000 entry directly, which is all the tree needs; GFS never recurses into
...
/// The cache root: `target/gfs-fixtures/<version>/`.
///
...
pub(crate) fn cache_root() -> PathBuf {
  ...
  base.join("gfs-fixtures").join(FIXTURE_VERSION)
}
