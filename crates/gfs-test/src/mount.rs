...
// Written out rather than derived because `Arc<Server>` and the watch sender
// are not `Debug`, and because a harness type's useful debug output is where it
// is listening and what repository it holds -- not its internals. The same
// applies to `Mount` and `Job` below. These became public, and so subject to the
// workspace's `missing_debug_implementations`, when the harness moved out of a
// `tests/` module and into this crate.
...
impl Backend {
  pub async fn start(fixture: &str) -> Backend {
    let (tmp, repo_path) = crate::scratch_clone(fixture).unwrap();
...
impl Mount {
  ...
  pub async fn with_configs(
    ...
    let snapshot_time = grant
      .snapshot_time
      .map(|t| gfs_types::Timestamp::new(t.secs, t.nanos))
      .unwrap();
