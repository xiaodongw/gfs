...
//! It is a measurement harness, not XVFS code, and it exists so the M3 decision
...
// ---------------------------------------------------------------------------
// The read side, against the real XVFS mount
// ---------------------------------------------------------------------------
...
impl Probe {
  fn attr(&self) -> FileAttr {
    ...
    FileAttr {
      ...
      perm: 0o644,
      nlink: 1,
      uid: xvfs_fuse::attr::Ownership::current().uid,
      gid: xvfs_fuse::attr::Ownership::current().gid,
      rdev: 0,
      blksize: 4096,
      flags: 0,
    }
...
impl ProbeMount {
  fn start(request_writeback: bool) -> ProbeMount {
    ...
    // Writable, unlike the XVFS mount: that is the whole point of the probe.
    config.mount_options = vec![
      fuser::MountOption::FSName("xvfs-mmap-probe".to_owned()),
