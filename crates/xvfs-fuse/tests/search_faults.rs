...
use harness::{Backend, TOKEN};
use tonic::{Request, Response, Status};
use xvfs_fuse::client::{MountBinding, SnapshotClient};
use xvfs_proto::v1;
use xvfs_proto::{SearchService, SearchServiceServer};
use xvfs_search::{exit_code, ExecutionStatus, SearchOutcome};
use xvfs_types::{HashAlgorithm, ObjectId, RepositoryId, Timestamp};
...
async fn prepared_head(backend: &Backend) -> ObjectId {
  let commit = backend
    ...
    .unwrap()
    .resolve(xvfs_types::RevisionSelector::parse("main", HashAlgorithm::Sha1).unwrap())
    .await
    .unwrap()
    .commit;
  let outcome = backend
    ...
    .await
    .unwrap();
  assert!(matches!(outcome, xvfs_server::PrepareOutcome::Ready(_)));
  commit
}
