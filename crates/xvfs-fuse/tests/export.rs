...
//! M2 built, and compare. Every part of the pipeline that XVFS wrote is on one
//! side of that comparison and nothing XVFS wrote is on the other.
//!
...
fn status_of(response: Response) -> xvfs_fuse::control::StatusReport {
  match response {
...
fn patch_of(response: Response) -> Vec<u8> {
  match response {
    Response::Diff { patch_b64url } => xvfs_types::path::b64url_decode(&patch_b64url).unwrap(),
...
async fn status_reports_every_shape_of_change_from_the_journal_alone() {
  ...
  let report = status_of(job.call(Request::Status).await);
  let kinds: Vec<(String, xvfs_overlay::ChangeKind)> = report
    .status
    .changes
    .iter()
    .map(|c| {
...
async fn an_export_applied_to_the_pinned_commit_reproduces_the_workspace_tree() {
  ...
  let (applied, expected) = on_fs(move || {
    ...
    // patch. Neither is XVFS code, which is the point: the oracle must not share
    // an implementation with the thing under test.
    let hex = commit.split_once(':').map(|(_, h)| h).unwrap().to_owned();
    xvfs_test::materialize_raw(&repo, &hex, &checkout).unwrap();
    ...
    (
      xvfs_test::snapshot_tree(&checkout).unwrap(),
      xvfs_test::snapshot_tree(&workspace).unwrap(),
    )
  })
  .await;
  ...
  let differences = xvfs_test::diff_trees(&comparable(applied), &comparable(expected));
  assert!(
...
/// than papered over.
fn comparable(tree: xvfs_test::TreeSnapshot) -> xvfs_test::TreeSnapshot {
  let directories: Vec<Vec<u8>> = tree
    .iter()
    .filter(|(_, entry)| matches!(entry, xvfs_test::EntrySnapshot::Directory))
...
fn shim() -> &'static Path {
  Path::new(env!("CARGO_BIN_EXE_xvfs-git-shim"))
}
