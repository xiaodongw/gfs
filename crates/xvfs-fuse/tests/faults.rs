...
//!
//! The crash matrix in `xvfs-test/tests/overlay_crash.rs` covers what a dead
...
use harness::{on_fs, read_dir_names, Backend, Job, Mount};
...
async fn concurrent_writers_to_one_file_leave_it_consistent() {
  // Not a test of who wins -- POSIX does not say, and neither does XVFS. What it
  ...
  assert_eq!(
    mount
      .overlay
      .get(&xvfs_types::BytePath::new(b"contested.txt".to_vec()))
      .unwrap()
      .size,
    4096
  );
...
async fn unmounting_with_open_descriptors_does_not_lose_a_committed_write() {
  ...
  let overlay = xvfs_overlay::Overlay::open(
    &job.state_dir.join("overlay").join("1"),
    &xvfs_overlay::Binding {
