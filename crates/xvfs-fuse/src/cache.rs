...
use xvfs_types::{HashAlgorithm, ObjectId, RepositoryId};
...
pub fn hash_blob(algorithm: HashAlgorithm, content: &[u8]) -> Result<ObjectId, XvfsError> {
  match algorithm {
    HashAlgorithm::Sha1 => {
      ...
      ObjectId::from_raw(HashAlgorithm::Sha1, &hasher.finalize())
        .map_err(|e| XvfsError::internal(format!("hashing a blob: {e}")))
    }
    HashAlgorithm::Sha256 => Err(XvfsError::new(
...
impl BlobCache {
  ...
  pub fn open(
    ...
    std::fs::create_dir_all(&root)
      .map_err(|e| XvfsError::internal(format!("creating the blob cache directory: {e}")))?;
  ...
  fn adopt_existing(&self) -> Result<(), XvfsError> {
    ...
    for shard in iter.flatten() {
      ...
      if prefix.len() != 2 || !xvfs_types::oid::is_hex(&prefix) {
        continue;
      }
      ...
      for file in files.flatten() {
        ...
        if !meta.is_file() || !xvfs_types::oid::is_hex(&name) {
  ...
  pub async fn open_blob(
    ...
    oid: &ObjectId,
    ticket: &str,
  ) -> Result<(PathBuf, Hydration), XvfsError> {
  ...
  async fn ensure(
    ...
    if let Some(mut receiver) = receiver {
      ...
      return match receiver.recv().await {
        ...
        Ok(Err(message)) => Err(XvfsError::new(ErrorCode::Unavailable, message)),
        // The fetcher died without sending. Retryable: the next attempt becomes
        // the fetcher itself.
        Err(_) => Err(XvfsError::new(
          ErrorCode::Unavailable,
  ...
  async fn fetch_and_publish(
    ...
    let directory = destination
      .parent()
      .ok_or_else(|| XvfsError::internal("cache path has no parent"))?;
    std::fs::create_dir_all(directory)
      .map_err(|e| XvfsError::internal(format!("creating a cache shard: {e}")))?;
    restrict(directory)?;
    ...
    let temporary = directory.join(format!(".{}.{}", oid.to_hex(), std::process::id()));
    {
      let mut file = std::fs::File::create(&temporary)
        .map_err(|e| XvfsError::internal(format!("creating a cache temporary: {e}")))?;
      file
        .write_all(&bytes)
        .map_err(|e| XvfsError::internal(format!("writing a cache temporary: {e}")))?;
      ...
      file
        .sync_all()
        .map_err(|e| XvfsError::internal(format!("syncing a cache temporary: {e}")))?;
      let mut permissions = file
        .metadata()
        .map_err(|e| XvfsError::internal(format!("stat on a cache temporary: {e}")))?
        .permissions();
    ...
    std::fs::rename(&temporary, &destination).map_err(|e| {
      ...
      XvfsError::internal(format!("publishing a cached blob: {e}"))
    })?;
...
fn restrict(path: &Path) -> Result<(), XvfsError> {
  std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o700))
    .map_err(|e| XvfsError::internal(format!("restricting cache directory permissions: {e}")))
}
