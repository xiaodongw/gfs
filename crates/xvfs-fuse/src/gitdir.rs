...
//! is the exact cost XVFS exists to avoid.
//!
//! # Six entries, not four
//!
//! DESIGN.md listed `HEAD`, `packed-refs`, `config`, and `xvfs.json`. ADR 0005
...
impl GitDir {
  pub fn new(facts: &GitDirFacts) -> Self {
    ...
    nodes.insert(
      b".git/xvfs.json".to_vec(),
      SynthNode::File(Arc::new(xvfs_json(facts))),
    );
    GitDir { nodes }
  }
...
///
/// The `[xvfs]` section is inert to Git — an unknown section is ignored — and
/// gives a human reading `.git/config` the same facts `xvfs.json` carries.
...
fn xvfs_json(facts: &GitDirFacts) -> Vec<u8> {
  let value = serde_json::json!({
    "api_version": xvfs_types::API_VERSION,
...
mod tests {
  ...
  #[test]
  fn xvfs_json_is_valid_json_naming_the_pinned_commit() {
    let bytes = xvfs_json(&facts(Some("refs/heads/main")));
