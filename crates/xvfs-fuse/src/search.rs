...
use xvfs_overlay::{Content, Overlay, OverlayKind};
use xvfs_search::classify::{CorpusPolicy, ExclusionReason};
use xvfs_search::local::{IgnoreRules, LocalBudget, LocalPath};
use xvfs_search::query::{
...
impl OverlayView {
  ...
  fn place(&self, path: &[u8]) -> Option<Vec<u8>> {
    ...
    if self
      .masked
      .iter()
      .any(|prefix| xvfs_search::local::within_scope(path, prefix) && path != prefix.as_slice())
    {
      return None;
    }
    Some(path.to_vec())
...
) -> Result<(SearchOutcome, usize), XvfsError> {
...
async fn read_overlay_file(
  overlay: &Arc<Overlay>,
  path: &BytePath,
) -> Result<Option<Vec<u8>>, XvfsError> {
  let overlay = Arc::clone(overlay);
  let path = path.clone();
  tokio::task::spawn_blocking(move || {
    let xvfs_overlay::Resolution::Overlay(entry) = overlay.resolve(&path) else {
      return Ok(None);
    };
    ...
    std::io::Read::read_to_end(&mut file, &mut bytes)
      .map_err(|e| XvfsError::internal(format!("reading a local ignore file: {e}")))?;
    Ok(Some(bytes))
  })
  .await
  .map_err(|e| XvfsError::internal(format!("the ignore-file task failed: {e}")))?
}
...
async fn search_overlay(
  ...
  ignore: &IgnoreRules,
  request: &SearchRequest,
) -> Result<xvfs_search::LocalOutcome, XvfsError> {
  if view.locals.is_empty() {
    return Ok(xvfs_search::LocalOutcome::default());
  }
  let overlay = Arc::clone(overlay);
  ...
  tokio::task::spawn_blocking(move || {
    xvfs_search::search_local(
      ...
      |local| {
        ...
        let mut bytes = Vec::new();
        std::io::Read::read_to_end(&mut file, &mut bytes)
          .map_err(|e| XvfsError::internal(format!("reading local content: {e}")))?;
        Ok(Some(bytes))
      },
    )
  })
  .await
  .map_err(|e| XvfsError::internal(format!("the local search task failed: {e}")))?
}

/// Combine the two halves into one answer.
fn merge(
  base: SearchResult,
  local: xvfs_search::LocalOutcome,
  view: &OverlayView,
) -> SearchOutcome {
  let SearchResult {
...
/// Re-exported so the CLI does not have to name `xvfs_search` to build a report.
pub use xvfs_search::exit_code;
