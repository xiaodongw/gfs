...
//!
//! The exporter in `xvfs-overlay` is synchronous and network-free by design, so
...
use xvfs_overlay::export::BaseContent;
use xvfs_overlay::{Content, Overlay, OverlayError, Status};
  ...
  pub async fn fetch(
    ...
    for change in &status.changes {
      ...
      if let Some(oid) = &change.old_oid {
        ...
        if change.is_content_change() && change.kind != xvfs_overlay::ChangeKind::Renamed {
          let oid = ObjectId::parse_qualified(oid)
            .map_err(|e| XvfsError::internal(format!("unreadable base object id: {e}")))?;
...
impl BaseContent for PreloadedBase {
  fn read(&self, oid: &ObjectId, path: &BytePath) -> xvfs_overlay::Result<Vec<u8>> {
...
async fn read_blob(
  ...
  let bytes = std::fs::read(&cached).map_err(|e| {
    XvfsError::internal(format!("reading a cached blob for {}: {e}", path.escaped()))
  })?;
