...
pub trait MountPublisher: Send + Sync + std::fmt::Debug {
  ...
  fn publish(&self, generation: &Path) -> Result<(), XvfsError>;
  /// Remove the workspace. Idempotent.
  fn unpublish(&self) -> Result<(), XvfsError>;
  /// The path the job sees.
  fn workspace(&self) -> &Path;
  /// A one-line description for `xvfs inspect`, so an operator can tell which
...
impl SymlinkPublisher {
  ...
  pub fn new(workspace: PathBuf) -> Result<Self, XvfsError> {
...
impl MountPublisher for SymlinkPublisher {
  fn publish(&self, generation: &Path) -> Result<(), XvfsError> {
    let temporary = self
      .workspace
      .with_extension(format!("xvfs-publish-{}", std::process::id()));
    let _ = std::fs::remove_file(&temporary);
    std::os::unix::fs::symlink(generation, &temporary)
      .map_err(|e| XvfsError::internal(format!("staging the workspace symlink: {}", e.kind())))?;
    // The atomic step. A reader either resolves the old target or the new one.
    std::fs::rename(&temporary, &self.workspace).map_err(|e| {
      let _ = std::fs::remove_file(&temporary);
      XvfsError::internal(format!("publishing the workspace: {}", e.kind()))
    })?;
    Ok(())
  }
  ...
  fn unpublish(&self) -> Result<(), XvfsError> {
    match std::fs::remove_file(&self.workspace) {
      ...
      Err(e) => Err(XvfsError::internal(format!(
...
mod tests {
  ...
  fn a_descriptor_opened_before_the_swap_still_reads_the_old_generation() {
    // The property `xvfs refresh` depends on: open handles keep their generation.
    use std::io::Read;
    let tmp = tempfile::tempdir().unwrap();
    let one = tmp.path().join("generations/1");
    let two = tmp.path().join("generations/2");
