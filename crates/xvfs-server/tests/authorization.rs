...
impl Harness {
  ...
  async fn mount_then_force_push_away(&self) -> (xvfs_types::MountGrant, String) {
    let grant = self
      .mounts
    ...
    let older = xvfs_test::git(&self.repo_path, &["rev-parse", "v1.0"])
      .unwrap()
      .trim()
      .to_owned();
    xvfs_test::git(&self.repo_path, &["update-ref", "refs/heads/main", &older]).unwrap();
    xvfs_test::git(&self.repo_path, &["update-ref", "-d", "refs/heads/feature"]).unwrap();
    xvfs_test::git(&self.repo_path, &["tag", "-d", "v2.0"]).unwrap();
    xvfs_test::git(&self.repo_path, &["tag", "-d", "tree-tag"]).unwrap();
...
async fn an_unauthorized_request_is_refused_before_the_repository_is_touched() {
  ...
  h.catalog
    .create_repository(&NewRepository {
      ...
      repo_path: std::path::PathBuf::from("/nonexistent/xvfs/definitely-not-here.git"),
      algorithm: HashAlgorithm::Sha1,
      upstream_url: None,
      credential_ref: None,
    })
...
async fn a_blob_ticket_is_bound_to_its_subject_repository_and_blob() {
  ...
  let blob = h
    .repo()
    .entry(&commit, &xvfs_types::BytePath::new("README.md"))
    .unwrap()
    .unwrap()
    .oid;
...
async fn the_git_path_does_reach_a_lease_retained_commit_as_adr_0002_records() {
  ...
  // mount-capability rule is scoped to the XVFS APIs and "does not extend to the
  ...
  // The XVFS API denies the stranger, as the tests above establish.
  assert_eq!(
    h.authz
      .authorize_commit(
        &h.stranger,
  ...
  let fetched = xvfs_test::git(
    ...
    &[
      ...
      "git -c uploadpack.hideRefs=refs/xvfs/ -c uploadpack.allowAnySHA1InWant=false upload-pack",
  ...
  let present = xvfs_test::git(&dest, &["cat-file", "-t", &grant.commit.to_hex()]).unwrap();
