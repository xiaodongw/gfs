...
async fn start() -> Fixture {
  let (tmp, repo_path) = xvfs_test::scratch_clone("basic").unwrap();
  ...
  let (bytes_tmp, bytes_path) = xvfs_test::scratch_clone("bytes").unwrap();
...
async fn resolve_revision_returns_the_pinned_commit_and_a_sanitized_time() {
  ...
  let expected = xvfs_test::git(&f.repo_path, &["rev-parse", "main"])
    .unwrap()
    .trim()
    .to_owned();
  ...
  assert!(t.secs >= xvfs_types::time::MIN_SUPPORTED_UNIX_SECS);
  assert!(t.secs <= xvfs_types::Timestamp::now().secs);
}
...
async fn the_reserved_namespace_is_refused_with_its_own_code() {
  let f = start().await;
  let mut c = client(&f, OWNER_TOKEN).await;
  for name in ["refs/xvfs/mounts/m-1", "xvfs/mounts/m-1"] {
    let status = c
      .resolve_revision(v1::ResolveRevisionRequest {
        repository_id: f.repo_id.to_string(),
      ...
      .await
      .unwrap_err();
    // Mapped to INVALID_ARGUMENT over gRPC, and distinguishable through the XVFS
    ...
    assert_eq!(
      status.metadata().get("xvfs-error-code").unwrap(),
      "RESERVED_NAMESPACE",
      "{name}"
    );
  }
...
async fn list_directory_pages_and_clamps_an_oversized_page_size() {
  ...
  let expected = xvfs_test::git(&f.repo_path, &["ls-tree", "--name-only", "HEAD"])
    .unwrap()
    .lines()
    .count();
  ...
  let clamped = c
    ...
    .unwrap()
    .into_inner();
  assert!(clamped.entries.len() <= xvfs_types::limits::MAX_DIRECTORY_PAGE_SIZE);
}
...
async fn a_capability_reaches_a_commit_that_a_force_push_made_unreachable() {
  ...
  let older = xvfs_test::git(&f.repo_path, &["rev-parse", "v1.0"])
    .unwrap()
    .trim()
    .to_owned();
  xvfs_test::git(&f.repo_path, &["update-ref", "refs/heads/main", &older]).unwrap();
  xvfs_test::git(&f.repo_path, &["update-ref", "-d", "refs/heads/feature"]).unwrap();
  xvfs_test::git(&f.repo_path, &["tag", "-d", "v2.0"]).unwrap();
  xvfs_test::git(&f.repo_path, &["tag", "-d", "tree-tag"]).unwrap();
...
async fn file_by_revision_returns_raw_bytes_and_the_resolved_commit() {
  let f = start().await;
  let path_b64 = xvfs_types::path::b64url_encode(b"README.md");
  let url = format!(
  ...
  let expected = xvfs_test::git(&f.repo_path, &["rev-parse", "main"])
    ...
    .to_owned();
  assert_eq!(
    headers.get("x-xvfs-commit").unwrap(),
    &format!("sha1:{expected}")
  );
  assert_eq!(headers.get("x-xvfs-mode").unwrap(), "100644");
  assert!(headers.get("x-xvfs-blob-oid").is_some());
...
async fn a_non_utf8_path_survives_the_base64url_query_parameter() {
  ...
  let path_b64 = xvfs_types::path::b64url_encode(name);
  let url = format!(
    "{}/v1/repos/{}/file?rev=main&path_b64url={path_b64}",
    f.http, f.bytes_id
  );
...
async fn the_immutable_blob_endpoint_requires_a_ticket() {
  ...
  let bad = format!(
    "{}/v1/repos/{}/blobs/{}?ticket=xvfs1.AAAA.AAAA",
...
async fn an_http_error_body_carries_the_stable_code_and_the_request_id() {
  let f = start().await;
  let path_b64 = xvfs_types::path::b64url_encode(b"no/such/file");
  let url = format!(
    "{}/v1/repos/{}/file?rev=main&path_b64url={path_b64}",
    f.http, f.repo_id
  );
...
async fn a_webhook_records_a_ref_event_idempotently() {
  ...
  let f = start().await;
  let commit = xvfs_test::git(&f.repo_path, &["rev-parse", "main"])
    .unwrap()
    .trim()
    .to_owned();
  ...
  let (status, _) = post(format!(
    r#"{{"ref_name":"refs/xvfs/mounts/m-1","new_oid":"sha1:{commit}"}}"#
  ))
  .await;
  assert_eq!(status, http::StatusCode::BAD_REQUEST);
}
