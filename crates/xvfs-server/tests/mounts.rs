...
impl Harness {
  ...
  async fn create(&self) -> xvfs_types::MountGrant {
    self
      .mounts
...
async fn create_mount_pins_a_commit_and_anchors_it_before_returning() {
  ...
  let expected = xvfs_test::git(&h.repo_path, &["rev-parse", "main"])
    .unwrap()
    .trim()
    .to_owned();
  ...
  assert_eq!(lease.state, xvfs_types::LeaseState::Active);
  assert_eq!(lease.subject, h.subject);

  // The snapshot time is the sanitized one, not the raw committer time, and it is
  // in the past.
  assert!(grant.snapshot_time <= xvfs_types::Timestamp::now());
  assert!(grant.snapshot_time >= xvfs_types::Timestamp::MIN_SUPPORTED);
...
async fn a_mounted_commit_survives_a_force_push_and_a_full_gc() {
  ...
  xvfs_test::git(&h.repo_path, &["update-ref", "-d", "refs/heads/feature"]).unwrap();
  xvfs_test::git(&h.repo_path, &["tag", "-d", "v2.0"]).unwrap();
  xvfs_test::git(&h.repo_path, &["tag", "-d", "tree-tag"]).unwrap();
  xvfs_test::git(
    &h.repo_path,
...
async fn reconciliation_abandons_a_preparing_lease_and_removes_its_anchor() {
  ...
  let anchor = xvfs_types::revision::lease_anchor_ref(mount_id.as_str());
  let commit = {
...
async fn reconciliation_is_idempotent_and_leaves_a_healthy_lease_alone() {
  ...
  for _ in 0..3 {
    let outcome = h.mounts.reconcile().await.unwrap();
    assert_eq!(outcome, xvfs_server::ReconcileOutcome::default());
  }
  assert_eq!(
    h.catalog.get_lease(&grant.mount_id).unwrap().unwrap().state,
    xvfs_types::LeaseState::Active
...
async fn reconciliation_removes_an_anchor_with_no_catalog_record() {
  ...
  let orphan = xvfs_types::revision::lease_anchor_ref("m-orphan");
...
async fn the_reserved_namespace_cannot_be_mounted() {
  ...
  for name in [anchor.as_str(), "xvfs/mounts/m-1"] {
    assert_eq!(
