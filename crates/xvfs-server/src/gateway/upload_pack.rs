...
/// repositories XVFS exists to serve. Memory is bounded by the container's
...
impl GitProtocol {
  ...
  /// reconstructed by XVFS rather than copied out of the request.
...
impl UploadPack {
  ...
  pub fn new(repo: &Path, policy: UploadPackPolicy) -> Result<Self, XvfsError> {
  ...
  pub fn spawn(
    ...
    mode: Mode,
  ) -> Result<tokio::process::Child, XvfsError> {
  ...
  /// `blob:limit`".
  pub fn validate_request(&self, request: &[u8]) -> Result<(), XvfsError> {
        ...
        for prefix in &self.policy.hidden_ref_prefixes {
          if rest.trim().starts_with(prefix.as_str()) {
            return Err(XvfsError::new(
              ...
              "that ref namespace is internal to XVFS",
            ));
          }
        }
      }
  ...
  pub fn decompress_body(&self, body: &[u8]) -> Result<Vec<u8>, XvfsError> {
    use std::io::Read;
...
mod tests {
  ...
  fn protected_config_denies_unadvertised_wants_and_hides_the_lease_namespace() {
    ...
    assert!(config.contains("transfer.hideRefs=refs/xvfs/"));
  ...
  fn request_validation_rejects_the_filters_git_config_would_allow() {
    ...
    let err = pack
      .validate_request(&body("want-ref refs/xvfs/mounts/m-1\n"))
      .unwrap_err();
