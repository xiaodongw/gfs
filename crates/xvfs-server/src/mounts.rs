...
impl MountManager {
  ...
  pub async fn create_mount(
    ...
    let grant = tokio::task::spawn_blocking(move || -> Result<MountGrant, XvfsError> {
  ...
  pub async fn renew_mount(
    ...
    let lease = self
      .catalog
      .get_lease(mount_id)?
      .ok_or_else(|| XvfsError::not_found("no such mount"))?;
    ...
    tokio::task::spawn_blocking(move || -> Result<Lease, XvfsError> {
  ...
  pub async fn release_mount(&self, mount_id: &MountId) -> Result<Lease, XvfsError> {
    let lease = self
      .catalog
      .get_lease(mount_id)?
      .ok_or_else(|| XvfsError::not_found("no such mount"))?;
  ...
  pub async fn sweep(&self) -> Result<crate::catalog::LeaseSweepOutcome, XvfsError> {
    let catalog = Arc::clone(&self.catalog);
    let policy = self.policy;
  ...
  pub async fn reconcile(&self) -> Result<ReconcileOutcome, XvfsError> {
    ...
    for lease in leases {
      ...
      match lease.state {
        ...
        LeaseState::Active => {
          ...
          let r = Arc::clone(&repo);
          let repaired = tokio::task::spawn_blocking(move || -> Result<bool, XvfsError> {
...
fn all_anchor_refs(repo: &dyn GitRepository) -> Result<Vec<String>, XvfsError> {
