...
//! through [`xvfs_types::redact::path`] on the way out, so it is escaped and
...
use xvfs_types::{redact, BytePath, MountId, ObjectId, RepositoryId, SubjectId};
...
mod tests {
  ...
  fn an_audit_line_carries_the_fields_adr_0006_requires() {
    ...
    let commit = ObjectId::from_hex(xvfs_types::HashAlgorithm::Sha1, &"ab".repeat(20)).unwrap();
