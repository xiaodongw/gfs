...
use xvfs_types::path::{b64url_decode, b64url_encode};
use xvfs_types::{MountId, ObjectId, RepositoryId, SubjectId, Timestamp};
...
impl CapabilityKey {
  ...
  pub fn new(secret: Vec<u8>) -> Result<Self, XvfsError> {
    if secret.len() < 32 {
  ...
  pub fn generate() -> Result<Self, XvfsError> {
...
fn decode_fields(bytes: &[u8], expected_tag: u8, count: usize) -> Result<Vec<Vec<u8>>, XvfsError> {
  let bad = || XvfsError::new(ErrorCode::Unauthenticated, "malformed capability");
...
fn disassemble(key: &CapabilityKey, token: &str) -> Result<Vec<u8>, XvfsError> {
  let bad = || XvfsError::new(ErrorCode::Unauthenticated, "malformed capability");
...
impl MountCapability {
  ...
  pub fn verify(key: &CapabilityKey, token: &str, now: Timestamp) -> Result<Self, XvfsError> {
    ...
    let cap = MountCapability {
      ...
      expires_at: Timestamp::from_secs(i64::from_be_bytes(
        fields[4]
          .as_slice()
          .try_into()
          .map_err(|_| XvfsError::new(ErrorCode::Unauthenticated, "malformed capability"))?,
      )),
    };
    let deadline = cap
      .expires_at
    ...
    let ticket = BlobTicket {
      ...
      expires_at: Timestamp::from_secs(i64::from_be_bytes(
        fields[4]
          .as_slice()
          .try_into()
          .map_err(|_| XvfsError::new(ErrorCode::Unauthenticated, "malformed ticket"))?,
      )),
    };
    if now.secs > ticket.expires_at.secs {
      return Err(XvfsError::new(
        ErrorCode::Expired,
        "blob ticket has expired",
      ));
    }
    Ok(ticket)
  }
...
fn utf8(bytes: &[u8]) -> Result<String, XvfsError> {
