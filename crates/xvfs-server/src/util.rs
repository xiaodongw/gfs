...
///
/// Read from `/dev/urandom` rather than pulled from a crate. XVFS targets Linux
...
pub fn random_bytes(n: usize) -> Result<Vec<u8>, XvfsError> {
  use std::io::Read;
...
pub fn random_hex(bytes: usize) -> Result<String, XvfsError> {
  Ok(
    random_bytes(bytes)?
      .iter()
      .map(|b| format!("{b:02x}"))
...
/// [`xvfs_types::MountId`] because the value is interpolated into
/// `refs/xvfs/mounts/{id}`, and the randomness is there so a mount ID is not
...
pub fn new_repository_id() -> Result<String, XvfsError> {
...
pub fn join_error(e: tokio::task::JoinError) -> XvfsError {
  if e.is_cancelled() {
    XvfsError::new(ErrorCode::Cancelled, "the operation was cancelled")
  } else {
    XvfsError::new(ErrorCode::Internal, "the operation failed unexpectedly")
  }
}
...
mod tests {
  ...
  fn mount_ids_are_valid_refs_and_do_not_repeat() {
    ...
    for _ in 0..200 {
      let id = new_mount_id();
      let parsed = xvfs_types::MountId::parse(&id).expect("must be a valid mount id");
      let anchor = xvfs_types::revision::lease_anchor_ref(parsed.as_str());
      assert!(xvfs_types::revision::is_reserved_ref(&anchor));
