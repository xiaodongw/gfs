...
use xvfs_types::{revision, ObjectId, RepositoryId};
...
impl Catalog {
  ...
  ) -> Result<RefObservation, XvfsError> {
  ...
  ) -> Result<Vec<(String, RefObservation)>, XvfsError> {
  ...
  pub fn list_refs(&self, repository_id: &RepositoryId) -> Result<Vec<RefRecord>, XvfsError> {
    self.with_conn(|conn| {
      let mut stmt = conn
        .prepare(
  ...
  pub fn mark_ref_event_processed(&self, event_id: i64) -> Result<(), XvfsError> {
    let now = now_secs();
    self.with_conn(|conn| {
      conn
        .execute(
...
mod tests {
  ...
  fn reconciliation_recovers_a_missed_webhook_in_both_directions() {
    ...
    let actual = vec![
      ...
      (xvfs_types::revision::lease_anchor_ref("m-1"), oid("ee")),
    ];
