...
pub fn open(path: &std::path::Path) -> Result<Connection, XvfsError> {
...
pub fn open_in_memory() -> Result<Connection, XvfsError> {
...
-- The current tip of every ref XVFS mirrors.
...
pub fn db_error(e: rusqlite::Error) -> XvfsError {
  ...
  match e {
    ...
    {
      XvfsError::new(ErrorCode::Unavailable, "catalog is busy")
    }
    other => XvfsError::new(ErrorCode::Internal, format!("catalog error: {other}")),
  }
}
