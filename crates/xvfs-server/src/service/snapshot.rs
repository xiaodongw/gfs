...
impl SnapshotApi {
  ...
  fn algorithm(&self, repository_id: &RepositoryId) -> Result<HashAlgorithm, XvfsError> {
  ...
  async fn repo_context(
    ...
    ctx: &Ctx,
    repository_id: &str,
  ) -> Result<(RepositoryId, HashAlgorithm), XvfsError> {
  ...
  ) -> Result<(CommitAccess, HashAlgorithm), XvfsError> {
  ...
  async fn snapshot_time(
    ...
    commit: &ObjectId,
    committer_time: xvfs_types::Timestamp,
  ) -> Result<xvfs_types::Timestamp, XvfsError> {
  ...
  fn attach_ticket(
    &self,
    access: &CommitAccess,
    mut entry: xvfs_types::TreeEntryInfo,
    wanted: bool,
  ) -> xvfs_types::TreeEntryInfo {
...
impl SnapshotService for SnapshotApi {
  ...
  async fn batch_get_entry(
    ...
    let result = async {
      ...
      let mut validated: Vec<Result<BytePath, XvfsError>> = Vec::with_capacity(req.paths.len());
      for p in req.paths {
          ...
          Ok(_) => match found.next() {
            ...
            Some(Ok(None)) => v1::EntryResult {
              result: Some(v1::entry_result::Result::Error(v1::ErrorDetail::from(
                &XvfsError::not_found("no such path in this commit"),
              ))),
            },
            ...
            None => v1::EntryResult {
              result: Some(v1::entry_result::Result::Error(v1::ErrorDetail::from(
                &XvfsError::internal("missing batch result"),
              ))),
            },
          },
        };
    ...
    let result = async {
      ...
      let capability = crate::auth::MountCapability::issue(
        ...
        &crate::auth::MountCapability {
          ...
          expires_at: xvfs_types::Timestamp::from_secs(lease.expires_at),
        },
      );
      Ok((lease, capability))
    }
    ...
    match result {
      Ok((lease, capability)) => {
        let response = v1::RenewMountResponse {
          ...
          lease_expiry: Some(xvfs_types::Timestamp::from_secs(lease.expires_at).into()),
        };
        self.finish(
          Action::RenewMount,
          &ctx,
