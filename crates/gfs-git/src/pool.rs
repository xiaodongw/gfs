...
impl RepoPool {
  ...
  pub fn open(path: impl AsRef<Path>, max: usize) -> Result<Arc<Self>, GfsError> {
  ...
  pub fn checkout(self: &Arc<Self>) -> Result<PooledRepo, GfsError> {
...
fn open_repo(path: &Path) -> Result<git2::Repository, GfsError> {
