//! Conversions between wire types and `gfs-types` domain types.
//!
...
//!
//! * **Request direction** (`try_*`) validates and returns [`GfsError`], so a
...
fn required<T>(value: Option<T>, field: &str) -> Result<T, GfsError> {
  value.ok_or_else(|| GfsError::invalid(format!("missing required field `{field}`")))
}
...
/// problem.
pub fn try_oid(s: &str, algorithm: HashAlgorithm, field: &str) -> Result<ObjectId, GfsError> {
  if s.is_empty() {
...
pub fn try_selector(s: &str, algorithm: HashAlgorithm) -> Result<RevisionSelector, GfsError> {
...
impl v1::Signature {
  pub fn try_into_domain(self, field: &str) -> Result<Signature, GfsError> {
    Ok(Signature {
      name: self.name,
      email: self.email,
...
impl v1::ResolveRevisionResponse {
  pub fn try_into_domain(self, algorithm: HashAlgorithm) -> Result<ResolvedRevision, GfsError> {
    Ok(ResolvedRevision {
      commit: try_oid(&self.commit_oid, algorithm, "commit_oid")?,
      tree: try_oid(&self.tree_oid, algorithm, "tree_oid")?,
...
impl TryFrom<v1::SnapshotState> for SnapshotState {
  ...
  fn try_from(s: v1::SnapshotState) -> Result<Self, GfsError> {
    match s {
      ...
      v1::SnapshotState::Unspecified => Err(GfsError::new(
        ErrorCode::Internal,
...
impl From<&GfsError> for v1::ErrorDetail {
  fn from(e: &GfsError) -> Self {
    v1::ErrorDetail {
...
/// The metadata key carrying the stable GFS error code.
pub const ERROR_CODE_METADATA_KEY: &str = "gfs-error-code";
...
/// retry policies already understand, and as the stable GFS wire name in the
...
/// backend is down.
pub fn to_status(e: &GfsError) -> tonic::Status {
...
/// Recover the GFS error code from a gRPC status, falling back to the gRPC code.
///
/// The fallback matters for statuses that did not come from a GFS handler --
/// a proxy timeout, a transport reset -- which carry no metadata but still need to
/// map onto something a client can act on.
pub fn from_status(status: &tonic::Status) -> GfsError {
  let code = status
    .metadata()
  ...
  GfsError::new(code, status.message().to_owned())
}

impl v1::ErrorDetail {
  /// Recover a domain error from a per-item `ErrorDetail`.
  pub fn into_domain(self) -> GfsError {
    ...
    GfsError::new(code, self.message)
  }
}
...
mod tests {
  ...
  fn error_codes_that_share_a_grpc_code_stay_distinguishable() {
    ...
    let building = to_status(&GfsError::new(
      ErrorCode::SnapshotBuilding,
      "manifest building",
    ));
    let unavailable = to_status(&GfsError::new(ErrorCode::Unavailable, "backend down"));
  ...
  #[test]
  fn a_status_without_gfs_metadata_still_maps_to_a_usable_code() {
    // A proxy timeout or transport reset carries no GFS metadata, but a client
  ...
  fn error_detail_round_trips_every_code() {
    for code in [
      ...
      let e = GfsError::new(code, "detail");
