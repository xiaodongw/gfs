...
//!
//! Arming requires setting `XVFS_OVERLAY_FAULT` in the environment, which no
...
fn armed() -> Option<&'static str> {
  ...
  ARMED
    .get_or_init(|| std::env::var("XVFS_OVERLAY_FAULT").ok())
    .as_deref()
}
...
pub fn trip(name: &str) {
  if armed() == Some(name) {
    ...
    eprintln!("xvfs-overlay: injected crash at {name}");
    std::process::abort();
  }
}
