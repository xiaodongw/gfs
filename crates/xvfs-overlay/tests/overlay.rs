...
use xvfs_overlay::model::{test_binding, test_snapshot_time, BaseTree};
use xvfs_overlay::{BaseFacts, Condition, Content, Overlay, OverlayConfig, Resolution, Source};
use xvfs_types::{BytePath, EntryKind, HashAlgorithm, ObjectId, Timestamp};
...
fn a_mode_change_diverges_metadata_without_fetching_any_bytes() {
  ...
  let entry = overlay
    ...
    .unwrap();
  assert_eq!(entry.kind, xvfs_overlay::OverlayKind::Executable);
...
fn an_overlay_from_another_commit_is_refused_rather_than_merged() {
  ...
  // every answer quietly about the wrong tree. `xvfs refresh` depends on this.
  let tmp = tempfile::tempdir().unwrap();
  {
    let overlay = open(tmp.path());
    overlay
