...
//! [`xvfs_overlay::model::Model`], which is a `BTreeMap` and nothing else. After
...
use xvfs_overlay::model::{
  generate, merged_view, path_pool, test_binding, test_snapshot_time, BaseTree, Model, Rng,
};
use xvfs_overlay::{Overlay, OverlayConfig};
use xvfs_types::EntryKind;
...
fn describe(
  expected: &std::collections::BTreeMap<Vec<u8>, xvfs_overlay::model::Node>,
  got: &std::collections::BTreeMap<Vec<u8>, xvfs_overlay::model::Node>,
) -> String {
...
fn short(node: &xvfs_overlay::model::Node) -> String {
  format!(
    "{:?}({})",
    node.kind,
