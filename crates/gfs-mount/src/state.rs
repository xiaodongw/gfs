...
use gfs_types::{LeaseState, Timestamp};
...
pub struct MountState {
  ...
  /// The selector the job asked for. Kept because `gfs refresh` re-resolves it,
...
impl MountState {
  ...
  pub fn load(state_dir: &Path) -> Result<Self, GfsError> {
    ...
    let state: MountState = serde_json::from_slice(&bytes)
      .map_err(|e| GfsError::internal(format!("mount state is unreadable: {e}")))?;
    if state.state_format_version != gfs_types::STATE_FORMAT_VERSION {
      return Err(GfsError::new(
        ErrorCode::FailedPrecondition,
        format!(
          "mount state format {} was written by a different GFS version (this build speaks {})",
  ...
  pub fn store(&self, state_dir: &Path) -> Result<(), GfsError> {
    let path = MountState::path(state_dir);
    let temporary = state_dir.join(".mount.json.tmp");
    let mut bytes = serde_json::to_vec_pretty(self)
      .map_err(|e| GfsError::internal(format!("serializing mount state: {e}")))?;
    bytes.push(b'\n');
    {
      let mut file = std::fs::File::create(&temporary)
        .map_err(|e| GfsError::internal(format!("creating the mount state temporary: {e}")))?;
      file
        .set_permissions(std::fs::Permissions::from_mode(0o600))
        .map_err(|e| GfsError::internal(format!("restricting mount state permissions: {e}")))?;
      file
        .write_all(&bytes)
        .map_err(|e| GfsError::internal(format!("writing mount state: {e}")))?;
      file
        .sync_all()
        .map_err(|e| GfsError::internal(format!("syncing mount state: {e}")))?;
    }
    std::fs::rename(&temporary, &path)
      .map_err(|e| GfsError::internal(format!("publishing mount state: {e}")))?;
    Ok(())
  }
...
pub fn prepare_state_dir(state_dir: &Path) -> Result<(), GfsError> {
  ...
  std::fs::set_permissions(state_dir, std::fs::Permissions::from_mode(0o700))
    .map_err(|e| GfsError::internal(format!("restricting the state directory: {e}")))?;
  Ok(())
}
...
mod tests {
  ...
  fn sample(dir: &Path) -> MountState {
    MountState {
      ...
      lease: LeaseRecord {
        ...
        capability: "gfs1.secret".to_owned(),
      },
      daemon_pid: 4242,
    }
  }
  ...
  fn mount_state_round_trips() {
    ...
    assert_eq!(loaded.lease.capability, "gfs1.secret");
  }
  ...
  fn a_state_file_from_another_format_version_is_refused_not_misread() {
    ...
    state.state_format_version = gfs_types::STATE_FORMAT_VERSION + 1;
    state.store(tmp.path()).unwrap();
