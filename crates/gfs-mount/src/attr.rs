...
use gfs_types::{EntryKind, Timestamp};
...
pub fn attr_of(record: &Record, snapshot_time: Timestamp, owner: Ownership) -> FileAttr {
  let (kind, perm, size, nlink) = match &record.node {
    Node::Base(entry) => match entry.kind {
      ...
      // A submodule: an empty, read-only directory with inspectable GFS
...
pub fn errno_of(error: &GfsError) -> fuser::Errno {
  use fuser::Errno;
  match error.code {
...
mod tests {
  use super::*;
  use gfs_types::{BytePath, HashAlgorithm, ObjectId, TreeEntryInfo};
  ...
  fn a_transport_failure_never_looks_like_a_missing_file() {
    ...
    for code in [
      ...
      let errno = errno_of(&GfsError::new(code, "x"));
      assert_eq!(errno, fuser::Errno::EIO, "{code:?}");
    }
    assert_eq!(errno_of(&GfsError::not_found("x")), fuser::Errno::ENOENT);
  }
