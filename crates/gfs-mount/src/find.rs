...
//!
//! The `find` half of what `gfs rg` is for content: an agent asking "which files
...
//!
//! A base path is placed through `OverlayView::place` -- named, not linked,
//! because it is `pub(crate)` -- so a renamed file is
...
pub async fn find(
  ...
  overlay: &Arc<Overlay>,
  request: &FindRequest,
) -> Result<(Vec<BytePath>, bool), GfsError> {
  ...
  let includes: Vec<gfs_search::Glob> = request
    .include_globs
    .iter()
    .map(|g| gfs_search::Glob::new(g))
    .collect();
  let excludes: Vec<gfs_search::Glob> = request
    .exclude_globs
    .iter()
    .map(|g| gfs_search::Glob::new(g))
    .collect();
  ...
  for entry in base {
    if let Some(placed) = view.place(entry.path.as_bytes()) {
      ...
      if matches(&placed) && gfs_search::local::within_scope(&placed, request.scope.as_slice()) {
        paths.push(placed);
      }
    }
  }
  for local in view.local_paths() {
    if matches(&local.path)
      && gfs_search::local::within_scope(&local.path, request.scope.as_slice())
    {
