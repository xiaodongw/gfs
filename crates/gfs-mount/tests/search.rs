...
use gfs_mount::search::{search, SearchRequest};
use gfs_search::query::{ExecutionStatus, SearchOutcome};
use gfs_test::mount::{Backend, Mount};
...
fn completion(outcome: &SearchOutcome) -> &gfs_search::Completion {
  match outcome {
...
async fn a_local_binary_file_is_a_reported_coverage_gap() {
  ...
  assert_eq!(
    gfs_search::exit_code(&outcome, true),
    4,
    "--require-exhaustive must fail on a local coverage gap too"
  );
  assert_eq!(gfs_search::exit_code(&outcome, false), 1);
}
