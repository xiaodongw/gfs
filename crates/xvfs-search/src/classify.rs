...
//!   to be ripgrep's rule, because `xvfs-rg` claims to substitute for `rg` and a
//!   different corpus boundary would make that claim false on exactly the files
//!   where it matters.
//! * **oversized** is 8 MiB (`xvfs_types::limits::MAX_SEARCHABLE_BLOB_BYTES`).
...
pub struct CorpusPolicy {
  ...
  /// that XVFS search substitutes for `rg`.
...
mod tests {
  ...
  fn a_nul_past_the_probe_window_is_not_seen_because_ripgrep_does_not_see_it() {
    // Not an oversight. `xvfs-rg` substitutes for `rg`, and a wider window would
