...
//! not states. `xvfs_types::SnapshotState` holds the vocabulary; this module
//! holds the machine.
//!
...
use xvfs_types::{ObjectId, RepositoryId, SnapshotState};
...
impl Cancel {
  ...
  pub fn check(&self) -> Result<(), XvfsError> {
...
impl SnapshotStore {
  ...
  pub fn generation(&self) -> Result<u64, XvfsError> {
  ...
  pub fn bump_generation(&self) -> Result<u64, XvfsError> {
    let repository = self.repository.as_str().to_owned();
  ...
  pub fn get(&self, commit: &ObjectId) -> Result<Option<SnapshotRecord>, XvfsError> {
    let repository = self.repository.as_str().to_owned();
  ...
  pub fn claim(&self, commit: &ObjectId, operation_id: &str) -> Result<Claim, XvfsError> {
    let repository = self.repository.as_str().to_owned();
    let commit_text = commit.to_qualified();
  ...
  pub fn record_progress(&self, commit: &ObjectId, progress: Progress) -> Result<(), XvfsError> {
    let repository = self.repository.as_str().to_owned();
    let commit_text = commit.to_qualified();
    self.store.with_tx(|tx| {
      tx.execute(
  ...
  pub fn complete(
    ...
    self
      .get(manifest.commit())?
      .ok_or_else(|| XvfsError::internal("the manifest just written is not readable"))
  }

  /// Record a failure, incrementing the attempt count.
  pub fn fail(&self, commit: &ObjectId, reason: &str) -> Result<(), XvfsError> {
    let repository = self.repository.as_str().to_owned();
    let commit_text = commit.to_qualified();
    self.store.with_tx(|tx| {
      tx.execute(
  ...
  pub fn abandon(&self, commit: &ObjectId) -> Result<(), XvfsError> {
    let repository = self.repository.as_str().to_owned();
    let commit_text = commit.to_qualified();
    self.store.with_tx(|tx| {
      tx.execute(
  ...
  pub fn manifest(&self, commit: &ObjectId) -> Result<Option<Manifest>, XvfsError> {
    let repository = self.repository.as_str().to_owned();
    let commit_text = commit.to_qualified();
  ...
  pub fn gc(&self, pinned: &HashSet<String>) -> Result<GcReport, XvfsError> {
    let repository = self.repository.as_str().to_owned();
    let now = now_secs();
    self.store.with_tx(|tx| {
  ...
  /// incremental base.
  pub fn ready_commits(&self) -> Result<Vec<ObjectId>, XvfsError> {
    let repository = self.repository.as_str().to_owned();
    self.store.with_conn(|conn| {
      let mut stmt = conn
        .prepare(
      ...
      for row in rows {
        ...
        out.push(
          ObjectId::parse_qualified(&text)
            .map_err(|e| XvfsError::internal(format!("stored commit id: {e}")))?,
        );
      }
      Ok(out)
    })
...
fn row_to_record(row: &rusqlite::Row<'_>) -> rusqlite::Result<Result<SnapshotRecord, XvfsError>> {
...
mod tests {
  ...
  use xvfs_types::{BytePath, HashAlgorithm};
