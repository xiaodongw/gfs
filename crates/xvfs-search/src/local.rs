...
//! the failure mode plainly — `xvfs search` must not become slower than the `rg`
...
//! what keeps `xvfs search` at zero base hydration on a clean workspace: with an
...
/// `info/exclude`, so on an XVFS mount that source is empty by construction —
...
pub fn search_local(
  ...
  budget: &LocalBudget,
  search_ignored: bool,
  mut read: impl FnMut(&LocalPath) -> Result<Option<Vec<u8>>, XvfsError>,
) -> Result<LocalOutcome, XvfsError> {
