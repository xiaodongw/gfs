...
use xvfs_types::{ObjectId, RepositoryId};
...
impl Default for IngestBudget {
  fn default() -> Self {
    IngestBudget {
      max_blob_bytes: xvfs_types::limits::MAX_SEARCHABLE_BLOB_BYTES,
...
impl BlobRegistry {
  ...
  /// as a searched one.
  pub fn intern(&self, facts: &[BlobFact]) -> Result<Vec<BlobKey>, XvfsError> {
    if facts.is_empty() {
      return Ok(Vec::new());
    }
  ...
  pub fn ingest(
    ...
    facts: &[BlobFact],
    budget: &IngestBudget,
    mut sink: impl FnMut(BlobKey, &[u8]) -> Result<(), XvfsError>,
  ) -> Result<IngestReport, XvfsError> {
  ...
  pub fn mark_indexed(&self, keys: &[BlobKey]) -> Result<(), XvfsError> {
    if keys.is_empty() {
      return Ok(());
    }
    let repository = self.repository.as_str().to_owned();
  ...
  pub fn record_for_oid(&self, oid: &ObjectId) -> Result<Option<BlobRecord>, XvfsError> {
    let repository = self.repository.as_str().to_owned();
  ...
  pub fn records_for_keys(&self, keys: &[BlobKey]) -> Result<Vec<BlobRecord>, XvfsError> {
    let repository = self.repository.as_str().to_owned();
  ...
  pub fn len(&self) -> Result<u64, XvfsError> {
    let repository = self.repository.as_str().to_owned();
    self.store.with_conn(|conn| {
      conn
        .query_row(
  ...
  pub fn is_empty(&self) -> Result<bool, XvfsError> {
    Ok(self.len()? == 0)
  }
  ...
  fn classified_keys(
    &self,
    keys: &[BlobKey],
  ) -> Result<std::collections::HashSet<BlobKey>, XvfsError> {
    let repository = self.repository.as_str().to_owned();
  ...
  fn apply_classifications(&self, updates: &[(BlobKey, ContentClass)]) -> Result<(), XvfsError> {
    if updates.is_empty() {
      return Ok(());
    }
    let repository = self.repository.as_str().to_owned();
...
fn row_to_record(row: &rusqlite::Row<'_>) -> rusqlite::Result<Result<BlobRecord, XvfsError>> {
    ...
    let class = match class {
      ...
      Some(name) => Some(
        ContentClass::from_wire_name(&name)
          .ok_or_else(|| XvfsError::internal(format!("unknown stored content class {name:?}")))?,
      ),
    };
    Ok(BlobRecord {
      key: to_key(key)?,
      oid: ObjectId::parse_qualified(&oid)
        .map_err(|e| XvfsError::internal(format!("stored object ID is unreadable: {e}")))?,
      size: size as u64,
      class,
      indexed: indexed != 0,
    })
...
fn to_key(value: i64) -> Result<BlobKey, XvfsError> {
...
mod tests {
  ...
  fn an_oversized_blob_is_excluded_without_being_read() {
    ...
    let facts = vec![BlobFact {
      oid: oid(9),
      size: xvfs_types::limits::MAX_SEARCHABLE_BLOB_BYTES + 1,
    }];
    let report = reg
