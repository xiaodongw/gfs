...
/// server implements it over `xvfs-git`; a test implements it over a `HashMap`.
///
...
pub trait BlobSource: Send + Sync {
  fn size(&self, oid: &xvfs_types::ObjectId) -> Result<u64, xvfs_types::XvfsError>;
  fn read(&self, oid: &xvfs_types::ObjectId) -> Result<Vec<u8>, xvfs_types::XvfsError>;
}
