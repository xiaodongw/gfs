...
//!
//! The process really dies. `xvfs-overlay-crash` calls `std::process::abort` at
...
use xvfs_overlay::fault::point;

fn harness() -> &'static Path {
  Path::new(env!("CARGO_BIN_EXE_xvfs-overlay-crash"))
}
...
impl Run {
  fn crashed(&self) -> bool {
    ...
    self.status.is_none() || self.status == Some(xvfs_overlay::fault::CRASH_STATUS)
  }
...
fn run(state: &PathBuf, fault: Option<&str>, args: &[&str]) -> Run {
  ...
  match fault {
    Some(name) => {
      command.env("XVFS_OVERLAY_FAULT", name);
    }
    None => {
      command.env_remove("XVFS_OVERLAY_FAULT");
    }
  }
