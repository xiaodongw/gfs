...
//!
//! ```text
//! xvfs-overlay-crash <state-dir> <op> [args...]
//! ```
//!
//! With `XVFS_OVERLAY_FAULT=<boundary>` set, the process aborts at that boundary
//! (see `xvfs_overlay::fault`). Without it, the operation completes and the
//! process exits 0 — the same binary is used for both, so "what recovery sees"
//! and "what success looks like" cannot drift apart.

use std::path::PathBuf;

use xvfs_overlay::model::{test_binding, test_snapshot_time};
use xvfs_overlay::{Overlay, OverlayConfig, Source};
use xvfs_types::BytePath;

fn main() {
  let args: Vec<String> = std::env::args().skip(1).collect();
  let [state_dir, op, rest @ ..] = args.as_slice() else {
    eprintln!("usage: xvfs-overlay-crash <state-dir> <op> [args...]");
    std::process::exit(2);
  };
  ...
  match op.as_str() {
    ...
    "materialize" => {
      let (target, bytes) = (&rest[0], rest[1].as_bytes());
      let facts = xvfs_overlay::BaseFacts {
        oid: xvfs_overlay::hash::blob_oid(xvfs_types::HashAlgorithm::Sha1, bytes)
          .expect("hashing the synthetic base blob"),
        kind: xvfs_types::EntryKind::Regular,
