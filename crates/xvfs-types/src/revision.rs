...
//! object-type semantics XVFS does not intend to expose. `main^{tree}` alone
...
//! reasoning ADR 0006 gives for rejecting `refs/xvfs/` at the lowest layer: a
...
pub const RESERVED_REF_PREFIX: &str = "refs/xvfs/";
...
impl RevisionSelector {
  ...
  pub fn parse(input: &str, algorithm: HashAlgorithm) -> Result<Self, XvfsError> {
    if input.is_empty() {
      return Err(XvfsError::invalid("revision selector is empty"));
    }
    ...
    // The qualified `algorithm:hex` form, which is what XVFS itself emits.
    ...
    // Namespace before well-formedness: a selector that names `refs/xvfs/` gets
    ...
    if let Some(rest) = input.strip_prefix("refs/") {
      ...
      if rest.is_empty() {
        return Err(XvfsError::invalid("ref name is just `refs/`"));
      }
...
fn unsupported_expression(input: &str) -> XvfsError {
...
fn reject_expression_syntax(input: &str) -> Result<(), XvfsError> {
...
///
/// Checking only the `refs/xvfs/` prefix is not enough. Git resolves a short
/// name by trying `refs/<name>` among other prefixes, so the short name
/// `xvfs/mounts/<id>` reaches `refs/xvfs/mounts/<id>` -- a lease anchor -- while
/// looking nothing like the reserved prefix. Both spellings are rejected here.
fn reject_reserved_namespace(input: &str) -> Result<(), XvfsError> {
...
fn validate_ref_name(input: &str) -> Result<(), XvfsError> {
...
mod tests {
  ...
  fn rejects_the_reserved_namespace_in_both_spellings() {
    ...
    // `xvfs/mounts/x` by trying `refs/xvfs/mounts/x`, which is a live lease
    // anchor.
    for bad in [
      "refs/xvfs/mounts/m-1",
      "xvfs/mounts/m-1",
      "refs/xvfs/anything",
      "xvfs/",
    ] {
