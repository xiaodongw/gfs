...
//! types. They live in `xvfs-types` rather than in `xvfs-git` because three
//! layers need them and none of them should depend on the Git implementation:
//! `xvfs-git` produces them, `xvfs-proto` converts them to and from the wire, and
//! `xvfs-server` authorizes and caches them.
