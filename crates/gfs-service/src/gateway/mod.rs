...
//!
//! DESIGN.md section 7.2 and ADR 0001 fix the shape: GFS does **not**
...
//! behaviour, filters, deltas, sideband -- and GFS owns everything around it.
//!
...
///
/// Bearer is what a GFS-configured client sends through
...
///
/// `gfs_types::b64url_decode` is the *url* alphabet and unpadded, so it cannot
...
async fn stream_child(
  ...
  let mut stdout = child
    .stdout
    .take()
    .ok_or_else(|| GfsError::new(ErrorCode::Internal, "upload-pack stdout was not piped"))?;
