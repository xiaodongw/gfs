...
//!
//! The mount-capability requirement is **scoped to the GFS APIs** -- snapshot,
...
use gfs_types::{ObjectId, RepositoryId, SubjectId, Timestamp};
...
pub trait Authenticator: Send + Sync + std::fmt::Debug {
  ...
  fn authenticate(&self, bearer: &str) -> Result<Identity, GfsError>;
}
  ...
  fn authenticate(&self, bearer: &str) -> Result<Identity, GfsError> {
    ...
    self
      ...
      .ok_or_else(|| {
        ...
        GfsError::new(ErrorCode::Unauthenticated, "invalid credential")
      })
  }
}
...
impl Authorizer {
  ...
  pub fn verify_blob_ticket(
    ...
    if &ticket.subject != subject || &ticket.repository_id != repository_id || &ticket.blob != blob
    {
      ...
      return Err(GfsError::masked_denial("no such blob"));
    }
    Ok(ticket)
  }
  ...
  pub fn authorize_mount_operation(
    &self,
    subject: &SubjectId,
    mount_id: &gfs_types::MountId,
    token: &str,
  ) -> Result<MountCapability, GfsError> {
    ...
    if &cap.subject != subject || &cap.mount_id != mount_id {
      return Err(GfsError::masked_denial("no such mount"));
    }
    Ok(cap)
  }
}
