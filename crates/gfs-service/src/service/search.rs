...
use gfs_proto::v1::{self, search_service_server::SearchService};
use gfs_search::query::{Budget, Query};
use gfs_search::Glob;
...
impl SearchService for SearchApi {
  ...
  async fn search(
    ...
    let result: Result<(crate::auth::CommitAccess, gfs_search::SearchResult), GfsError> = async {
...
fn build_query(req: &v1::SearchRequest) -> Result<Query, GfsError> {
  if req.pattern.is_empty() {
    return Err(GfsError::invalid("the search pattern is empty"));
  }
  if req.scope.len() > limits::MAX_PATH_BYTES {
    return Err(GfsError::invalid("the search scope is too long"));
  }
...
fn to_proto_match(m: &gfs_search::Match) -> v1::SearchMatch {
  v1::SearchMatch {
    path: m.path.clone(),
    line: m.line,
    column: m.column,
...
fn to_proto_completion(c: &gfs_search::Completion) -> v1::SearchCompletion {
  v1::SearchCompletion {
    execution_status: match c.execution_status {
      gfs_search::ExecutionStatus::Complete => v1::ExecutionStatus::Complete as i32,
      gfs_search::ExecutionStatus::Truncated => v1::ExecutionStatus::Truncated as i32,
    },
...
fn truncation_name(reason: gfs_search::TruncationReason) -> &'static str {
  use gfs_search::TruncationReason as T;
  match reason {
