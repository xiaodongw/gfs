...
impl Server {
  ...
  pub fn new(
    ...
    let store = Arc::new(
      gfs_search::SearchStore::open_in_memory().expect("an in-memory search index always opens"),
    );
  ...
  pub fn with_search_index(mut self, path: &std::path::Path) -> Result<Self, GfsError> {
    let store = Arc::new(gfs_search::SearchStore::open(path)?);
  ...
  /// read.
  pub async fn recover(&self) -> Result<(), GfsError> {
  ...
  pub fn check_pinned_versions() -> Result<(), GfsError> {
    const EXPECTED_LIBGIT2: &str = "1.9.6";
    let actual = gfs_git::format::libgit2_version();
