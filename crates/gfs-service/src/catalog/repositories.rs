...
use gfs_types::{time, DisplayName, HashAlgorithm, ObjectId, RepositoryId, Timestamp};
...
impl RepositoryState {
  ...
  pub fn parse(s: &str) -> Result<Self, GfsError> {
    match s {
...
impl Catalog {
  ...
  pub fn create_repository(&self, new: &NewRepository) -> Result<RepositoryRecord, GfsError> {
    let now = now_secs();
    self.with_tx(|tx| {
  ...
  pub fn get_repository(&self, id: &RepositoryId) -> Result<Option<RepositoryRecord>, GfsError> {
    self.with_conn(|conn| {
      conn
        .query_row(
  ...
  pub fn list_repositories(&self) -> Result<Vec<RepositoryRecord>, GfsError> {
    self.with_conn(|conn| {
      let mut stmt = conn
        .prepare(
          "SELECT repository_id, display_name, repo_path, state, algorithm,
  ...
  pub fn delete_repository(&self, id: &RepositoryId) -> Result<(), GfsError> {
    self.with_tx(|tx| {
      let live: i64 = tx
        .query_row(
      ...
      let n = tx
        ...
        .map_err(db_error)?;
      if n == 0 {
        return Err(GfsError::not_found("no such repository"));
      }
      Ok(())
    })
  }
  ...
  pub fn snapshot_time(
    ...
    repository_id: &RepositoryId,
    commit: &ObjectId,
  ) -> Result<Option<Timestamp>, GfsError> {
    self.with_conn(|conn| {
      conn
        .query_row(
          "SELECT snapshot_secs, snapshot_nanos FROM commits
...
fn row_to_repository(
  row: &rusqlite::Row<'_>,
) -> rusqlite::Result<Result<RepositoryRecord, GfsError>> {
...
mod tests {
  ...
  fn new_repo(id: &str) -> NewRepository {
    NewRepository {
      ...
      credential_ref: Some("secret-store://gfs/upstream/acme".to_owned()),
    }
  }
