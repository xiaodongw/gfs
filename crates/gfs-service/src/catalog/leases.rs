...
use gfs_types::{LeasePolicy, LeaseState, MountId, ObjectId, RepositoryId, SubjectId};
...
impl Catalog {
  ...
  pub fn activate_lease(&self, mount_id: &MountId) -> Result<(), GfsError> {
    let now = now_secs();
    self.with_tx(|tx| {
      let n = tx
        .execute(
      ...
      if n == 0 {
        ...
        return Err(match state {
          None => GfsError::not_found("no such lease"),
          Some(s) => GfsError::new(
  ...
  pub fn get_lease(&self, mount_id: &MountId) -> Result<Option<Lease>, GfsError> {
    self.with_conn(|conn| {
      conn
  ...
  pub fn renew_lease(
    ...
    self.with_tx(|tx| {
      let lease = tx
        ...
        .map_err(db_error)?
        .transpose()?
        .ok_or_else(|| GfsError::not_found("no such lease"))?;
  ...
  pub fn release_lease(&self, mount_id: &MountId) -> Result<Lease, GfsError> {
    let now = now_secs();
    self.with_tx(|tx| {
      let lease = tx
        .query_row(LEASE_SELECT, [mount_id.as_str()], row_to_lease)
  ...
  pub fn record_renewal_failure(&self, mount_id: &MountId) -> Result<u32, GfsError> {
    let now = now_secs();
    self.with_tx(|tx| {
      tx.execute(
  ...
  pub fn unreconciled_leases(&self) -> Result<Vec<Lease>, GfsError> {
    self.with_conn(|conn| {
      let mut stmt = conn
        .prepare(&format!(
  ...
  pub fn sweep_leases(&self, policy: &LeasePolicy) -> Result<LeaseSweepOutcome, GfsError> {
    let now = now_secs();
  ...
  pub fn forget_lease(&self, mount_id: &MountId) -> Result<(), GfsError> {
    self.with_conn(|conn| {
      conn
        .execute(
  ...
  pub fn abandon_lease(&self, mount_id: &MountId, reason: &str) -> Result<(), GfsError> {
    let now = now_secs();
    tracing::warn!(
      mount_id = %mount_id,
      reason,
...
mod tests {
  ...
  fn begin(f: &Fixture, id: &str, ttl: i64) -> Lease {
    ...
    let anchor = gfs_types::revision::lease_anchor_ref(id);
    f.cat
