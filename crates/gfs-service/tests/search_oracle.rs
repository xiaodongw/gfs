//! M4.6: GFS search against `rg`, over a generated query corpus.
//!
...
//! offset within the line — exactly the number GFS reports as `column - 1`. It
...
//! handling would be arbitrary, and GFS's base index has no notion of hidden
//! files.
//!
...
//! * **files over 8 MiB.** GFS excludes them and *reports* the exclusion;
...
use gfs_search::query::{ExecutionStatus, Query};
...
impl Subject {
  async fn new(fixture: &str) -> Subject {
    ...
    gfs_test::materialize_raw(&repo_path, &commit.to_hex(), &materialized).unwrap();
...
/// and `gfs_types::path::b64url_decode` would reject `+` and `/`.
...
/// where GFS and `rg` legitimately differ, and mixing those into a bulk
...
async fn generated_literal_queries_match_ripgrep_exactly() {
  ...
  for fixture in BULK_FIXTURES {
    ...
    for pattern in &patterns {
      let (ours, truncated) = subject.gfs(pattern, true, false).await;
      if truncated {
        continue;
      }
      ...
      assert_eq!(
        ours, theirs,
        "fixture {fixture}, literal {pattern:?}: GFS and rg disagree"
      );
      compared += 1;
      if !theirs.is_empty() {
        with_matches += 1;
...
async fn case_insensitive_queries_match_ripgrep_exactly() {
  ...
  for fixture in ["basic", "deep"] {
    ...
    for pattern in generated_literals(&subject.materialized, 20) {
      ...
      let (ours, truncated) = subject.gfs(&upper, true, true).await;
      if truncated {
        continue;
      }
      assert_eq!(
...
async fn regex_queries_match_ripgrep_exactly() {
  for fixture in ["basic", "modes", "deep"] {
    ...
    for pattern in REGEX_CORPUS {
      let (ours, truncated) = subject.gfs(pattern, false, false).await;
      if truncated {
...
async fn alternation_returns_every_branch_not_just_the_longest() {
  ...
  for pattern in ["println|util", "util|println", "README|guide|added"] {
    let (ours, _) = subject.gfs(pattern, false, false).await;
...
async fn non_utf8_paths_are_reported_identically() {
  ...
  let (ours, _) = subject.gfs("content", true, false).await;
...
async fn a_huge_line_is_searched_by_both() {
  ...
  let (ours, _) = subject
    .gfs("this-is-not-in-the-huge-line", true, false)
    .await;
...
async fn a_symlink_is_searched_by_neither() {
  ...
  // does not follow symlinks by default and the GFS corpus excludes them.
...
#[tokio::test]
async fn an_oversized_file_is_excluded_by_gfs_and_the_exclusion_is_reported() {
  // `content` holds a 12 MiB blob, over ADR 0004's 8 MiB cap. `rg` has no size
  // limit, so this is a real difference — and the point is that GFS *says so*
...
#[tokio::test]
async fn a_binary_file_is_excluded_by_both_and_reported_by_gfs() {
  let subject = Subject::new("content").await;
  let result = subject
    .server
    .search
  ...
  // it as text. GFS additionally reports it.
