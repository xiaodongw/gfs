...
//!
//! The oracle is stock `git ls-tree`, never libgit2 and never GFS reading
...
use gfs_search::snapshots::{Claim, PreparePolicy};
use gfs_search::{SearchStore, SnapshotStore};
use gfs_service::auth::{AllowList, CapabilityKey, StaticTokens};
use gfs_service::catalog::repositories::NewRepository;
use gfs_service::search::{IndexManager, PrepareOutcome};
...
impl Harness {
  ...
  fn git_paths(&self, commit: &ObjectId) -> BTreeSet<Vec<u8>> {
    use std::ffi::OsStr;
    let listing = gfs_test::git_bytes(
      &self.repo_path,
      &[
        OsStr::new("ls-tree"),
        OsStr::new("-r"),
...
async fn prepared(h: &Harness, commit: &ObjectId) -> gfs_search::Manifest {
...
async fn an_incremental_build_describes_the_same_tree_as_a_full_one() {
  ...
  let resolve = |manifest: &gfs_search::Manifest, blobs: gfs_search::BlobRegistry| {
...
async fn the_prepare_snapshot_rpc_reports_a_real_state() {
  ...
  let response =
    gfs_proto::v1::snapshot_service_server::SnapshotService::prepare_snapshot(&api, request)
      .await
      .unwrap()
      .into_inner();
