...
//! `gfs-git/tests/repository.rs` and `tests/mounts.rs`, and the
...
use gfs_service::auth::{AllowList, CapabilityKey, SnapshotAuthorization, StaticTokens};
...
async fn a_million_entry_snapshot_can_be_paged_one_directory_at_a_time() {
  ...
  let repo_path = gfs_test::big_tree(DIRS, FILES).unwrap();
  ...
  loop {
    ...
    for e in &page.entries {
      if e.kind == gfs_types::EntryKind::Directory {
  ...
  assert_eq!(
    gfs_test::expected_entries(DIRS, FILES),
    total,
...
async fn concurrent_ref_movement_cannot_produce_a_mixed_commit_response() {
  ...
  let tip = gfs_test::git(&h.repo_path, &["rev-parse", "main"])
    .unwrap()
    .trim()
    .to_owned();
  ...
  let writer = std::thread::spawn(move || {
    for i in 0..60 {
      ...
      let _ = gfs_test::git(&writer_path, &["update-ref", "refs/heads/main", target]);
...
async fn a_blob_ticket_cannot_be_used_to_probe_for_blobs_in_another_snapshot() {
  ...
  let absent = gfs_types::ObjectId::from_hex(HashAlgorithm::Sha1, &"ab".repeat(20)).unwrap();
  let for_existing = h
    .server
    .authz
