...
//!
//! Everything is answered from the mount, from `.git/gfs.json`, and from the
...
const SUPPORTED: &str = "\
gfs `git` shim: the supported grammar (ADR 0005) is
...
/// What `.git/gfs.json` says about the workspace.
struct Workspace {
  toplevel: PathBuf,
  git_dir: PathBuf,
  commit: String,
...
impl Workspace {
  ...
  /// `gfs.json`: a shim that answered for an ordinary Git repository would be
  ...
  fn discover() -> Result<Self, Failure> {
    ...
    while let Some(directory) = current {
      ...
      let descriptor = git_dir.join("gfs.json");
    ...
    fatal("not a GFS workspace (no .git/gfs.json in this directory or any parent)")
  }
...
fn status(workspace: &Workspace, args: &[String]) -> Result<(), Failure> {
  ...
  if porcelain {
    for change in &report.status.changes {
      ...
      // from HEAD" -- because that is what a GFS change *is*: there is no
  ...
  for change in &report.status.changes {
    let label = match change.kind {
      gfs_overlay::ChangeKind::Added => "new file",
      gfs_overlay::ChangeKind::Modified => "modified",
      gfs_overlay::ChangeKind::Deleted => "deleted",
      gfs_overlay::ChangeKind::TypeChanged => "typechange",
      gfs_overlay::ChangeKind::ModeChanged => "mode",
      gfs_overlay::ChangeKind::Renamed => "renamed",
    };
...
fn diff(workspace: &Workspace, args: &[String]) -> Result<(), Failure> {
  ...
  for arg in args {
    ...
    match arg.as_str() {
      ...
      // GFS has no index, so there is nothing staged and `--cached` is empty by
  ...
  let report = workspace.status()?;
  let selected: Vec<&gfs_overlay::status::Change> = report
    .status
    .changes
    .iter()
    .filter(|change| {
...
fn filter_patch(patch: &[u8], selected: &[&gfs_overlay::status::Change]) -> Vec<u8> {
