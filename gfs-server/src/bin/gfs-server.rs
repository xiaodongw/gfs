...
use gfs_service::{observability, service, Catalog, Server};
use gfs_types::{LeasePolicy, SubjectId};

#[derive(Parser, Debug)]
#[command(
  name = "gfs-server",
  version,
  about = "GFS repository and snapshot server"
)]
struct Args {
  /// Directory holding the catalog and bare repositories.
  #[arg(long, env = "GFS_STATE_DIR", default_value = "/var/lib/gfs")]
  state_dir: PathBuf,

  /// gRPC listen address.
  #[arg(long, env = "GFS_GRPC_ADDR", default_value = "127.0.0.1:8431")]
  grpc_addr: String,

  /// HTTP listen address for blobs, health, and metrics.
  #[arg(long, env = "GFS_HTTP_ADDR", default_value = "127.0.0.1:8430")]
  http_addr: String,
  ...
  #[arg(long, env = "GFS_CAPABILITY_KEY", hide_env_values = true)]
  ...
  #[arg(long, env = "GFS_DEV_TOKEN", hide_env_values = true)]
...
async fn main() -> anyhow::Result<()> {
  ...
  let (authenticator, policy) = match &args.dev_token {
    Some(token) => {
      ...
      (
        auth as Arc<dyn gfs_service::auth::Authenticator>,
        policy as Arc<dyn gfs_service::auth::RepositoryPolicy>,
      )
    }
    None => {
      ...
      (
        Arc::new(StaticTokens::new()) as Arc<dyn gfs_service::auth::Authenticator>,
        Arc::new(gfs_service::auth::policy::DenyAll)
          as Arc<dyn gfs_service::auth::RepositoryPolicy>,
      )
    }
  };
...
fn import_repository(server: &Server, id: &str, path: &std::path::Path) -> anyhow::Result<()> {
  ...
  if server.catalog.get_repository(&repository_id)?.is_none() {
    ...
    let format = gfs_git::read_format(&path)?;
