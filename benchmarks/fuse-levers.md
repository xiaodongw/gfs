...
| step | baseline | passthrough code, no cap |
| --- | ---: | ---: |
| mount | 0.270 s | 0.226 s |
| prewarm (waited) | – | – |
| read 2000 files, cold | 0.746 s | 0.740 s |
| read again, warm | 0.264 s | 0.265 s |
| `rg -F TODO`, first (1683 lines) | 0.654 s | 0.634 s |
| `rg -F TODO`, second | 0.158 s | 0.164 s |
| read largest blob, cold | 0.006 s | 0.006 s |
| read largest blob, warm | 0.005 s | 0.006 s |
| write 64 MiB, 4 KiB `dd` | 3.950 s | 4.260 s |
| `cp -r` 10225 files in | 29.383 s | 27.599 s |
| read the 64 MiB back | 0.048 s | 0.075 s |
| read the copied files back | 2.772 s | 2.799 s |
| `git status` after the writes | 0.728 s | 0.717 s |
| `git add -A` + commit | 4.919 s | 5.010 s |
| open+close, base blob | 63.5 µs | 63.7 µs |
| open+read+close, base blob | 65.9 µs | 65.2 µs |
| stat, cached | 1.8 µs | 1.9 µs |
| write 4 KiB, overlay file | 244.1 µs | 249.1 µs |
| open+close, overlay file | 72.1 µs | 72.6 µs |
| open+read+close, overlay file | 216.2 µs | 219.7 µs |
...
| step | baseline | passthrough code, no cap |
| --- | ---: | ---: |
| mount | 0.087 s | 0.091 s |
| prewarm (waited) | – | – |
| read 2000 files, cold | 0.695 s | 0.726 s |
| read again, warm | 0.255 s | 0.269 s |
| `rg -F TODO`, first (35 lines) | 0.218 s | 0.221 s |
| `rg -F TODO`, second | 0.082 s | 0.083 s |
| read largest blob, cold | 0.005 s | 0.005 s |
| read largest blob, warm | 0.005 s | 0.005 s |
| write 64 MiB, 4 KiB `dd` | 3.943 s | 3.970 s |
| `cp -r` 3688 files in | 11.345 s | 10.345 s |
| read the 64 MiB back | 0.049 s | 0.054 s |
| read the copied files back | 1.335 s | 1.345 s |
| `git status` after the writes | 0.448 s | 0.458 s |
| `git add -A` + commit | 8.976 s | 9.002 s |
| open+close, base blob | 62.9 µs | 64.1 µs |
| open+read+close, base blob | 66.3 µs | 65.6 µs |
| stat, cached | 2.0 µs | 1.9 µs |
| write 4 KiB, overlay file | 243.9 µs | 244.4 µs |
| open+close, overlay file | 72.7 µs | 73.1 µs |
| open+read+close, overlay file | 218.4 µs | 219.9 µs |
