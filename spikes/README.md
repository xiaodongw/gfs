...
maintained. They are deliberately kept out of the `gfs-*` production workspace
...
| --- | --- | --- |
| `git-probe` | M0.3 | Does libgit2 agree with stock Git on the repositories GFS will host, and where is the supported-format boundary? |
...
one file. Mirrors land in `$GFS_CORPUS_DIR` (default `~/gfs-corpus`), outside
