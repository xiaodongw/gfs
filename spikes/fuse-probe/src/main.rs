...
fn measure(
  ...
  out.push(m(
    "readlink",
    format!("link-to-first -> {}", link.display()),
  ));
  out.push(m(
    "statfs",
...
fn capabilities() -> Result<()> {
  ...
  for (label, acl, auto_unmount) in [
    ...
    let mut cfg = base_config(if label.starts_with("multithreaded") {
      8
    } else {
      1
    });
