...
pub fn search(
    ...
    'outer: for (path, _mode, key) in eligible.iter() {
        ...
        if line_start < content.len()
            && emit(&content[line_start..], line_no, &mut matches)
        {
            truncation = Some(TruncationReason::ResultLimit);
            break;
        }
...
mod tests {
    ...
    fn result(status: ExecutionStatus, gaps: bool, matches: usize) -> SearchOutcome {
        ...
        SearchOutcome::Completed(SearchResult {
            ...
            completion: Completion {
                ...
                truncation: (status == ExecutionStatus::Truncated)
                    .then_some(TruncationReason::ResultLimit),
                coverage: Coverage {
                    eligible_paths: 10,
    ...
    fn literal_runs_are_conservative_about_optionality() {
        assert_eq!(literal_runs("RequestContext"), vec![b"RequestContext".to_vec()]);
        ...
        assert_eq!(literal_runs("foo|bar"), vec![b"foo".to_vec(), b"bar".to_vec()]);
        assert_eq!(literal_runs(r"fn\s+authorize_"), vec![b"fn".to_vec(), b"authorize_".to_vec()]);
    }
