...
impl Manifest {
    ...
    pub fn storage(&self) -> ManifestStorage {
        ...
        let reverse_bytes: u64 = self
            .reverse
            .values()
            .map(|v| 2 + 4 * v.len() as u64)
            .sum();
        ManifestStorage {
...
pub fn build_snapshot(
    ...
    for (ordinal, (path, mode, oid)) in entries.into_iter().enumerate() {
        ...
        let blob = odb.read(oid).with_context(|| format!("reading blob {oid}"))?;
...
mod tests {
    ...
    fn classification_matches_the_documented_corpus() {
        ...
        assert_eq!(classify(&vec![b'x'; MAX_INDEXED_BYTES + 1]), Some(Exclusion::Oversized));
