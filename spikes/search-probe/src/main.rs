...
fn main() -> Result<()> {
    match Cli::parse().cmd {
        ...
        Cmd::Manifests { repo, rev, commits, retained, json } => {
            manifests(&repo, &rev, commits, retained, json)
        }
        Cmd::Query { repo, rev, pattern, regex, path_prefix, limit, require_exhaustive } => {
            run_query(&repo, &rev, &pattern, regex, path_prefix, limit, require_exhaustive)
        }
        Cmd::Verify { repo, rev, rg, patterns } => verify(&repo, &rev, rg, &patterns),
...
fn build(repo_path: &Path, rev: &str, json: Option<PathBuf>) -> Result<()> {
    ...
    println!("manifest bitmap       {:.1} KiB", storage.bitmap as f64 / 1024.0);
    println!("manifest reverse      {:.2} MiB", mib(storage.reverse_table));
...
fn manifests(
    ...
    println!();
    println!("mean manifest per snapshot   {:.2} MiB", avg_total / 1048576.0);
    println!("  path table                 {:.2} MiB", mean(|s| s.path_table) / 1048576.0);
    println!("  membership bitmap          {:.1} KiB", mean(|s| s.bitmap) / 1024.0);
    println!("  reverse table              {:.2} MiB", mean(|s| s.reverse_table) / 1048576.0);
...
fn run_query(
    ...
    let input = query::SearchInput {
        ...
        budget: query::Budget { max_results: limit, ..Default::default() },
        index_generation: 1,
    };
...
fn verify(repo_path: &Path, rev: &str, rg: Option<PathBuf>, patterns: &[String]) -> Result<()> {
    ...
    for pat in patterns {
        let input = query::SearchInput {
            ...
            literal: true,
            path_prefix: None,
            budget: query::Budget { max_results: usize::MAX, ..Default::default() },
            index_generation: 1,
        };
