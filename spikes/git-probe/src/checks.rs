...
pub fn run_all(fixture: &str, bare: &Path) -> Vec<CheckResult> {
    ...
    let repo = match Libgit2Repository::open(bare, 4) {
        ...
        Err(e) => {
            ...
            let expected = matches!(fixture, "reftable")
                || (fixture == "sha256" && !gitrepo::build_has_sha256());
...
fn check_tree_walk_matches_stock(
    ...
    if expected.len() != actual.len() {
        return Ok(fail(
            ...
            format!("entry count: stock={} libgit2={}", expected.len(), actual.len()),
        ));
    }
    ...
    Ok(ok(
        fixture,
        "tree_walk_matches_stock",
        format!("{} entries identical in path bytes, mode, and OID", actual.len()),
    ))
}
    ...
    for (path, mode, oid) in entries.iter().take(200) {
        ...
        if actual != expected.stdout {
            return Ok(fail(
                ...
                format!("{} differs from cat-file", BytePath::new(path.clone()).escaped()),
            ));
        }
        compared += 1;
        bytes += actual.len();
...
fn check_gitlink_mode(
    ...
    for path in &gitlinks {
        let e = repo
            .entry(&commit, &BytePath::new(path.clone()))?
            .ok_or_else(|| anyhow!("gitlink {:?} missing", BytePath::new(path.clone()).escaped()))?;
        ...
        if !children.is_empty() {
            return Ok(fail(
                fixture,
                "gitlink_mode",
                format!("gitlink {} listed {} children", BytePath::new(path.clone()).escaped(), children.len()),
            ));
        }
    }
    Ok(ok(
        fixture,
        "gitlink_mode",
        format!("{} gitlink(s) classified as 160000 and not traversed", gitlinks.len()),
    ))
}
...
fn check_directory_pagination(
    ...
    if deduped.len() != seen.len() {
        return Ok(fail(
            fixture,
            "directory_pagination",
            format!("{} duplicate entries across pages", seen.len() - deduped.len()),
        ));
    }
...
fn check_tree_diff_matches_stock(
    ...
    let stock_raw = g(bare, &["diff-tree", "-r", "--name-status", "HEAD~1", "HEAD"])?;
...
fn check_object_creation_round_trips(
    ...
    let fsck = std::process::Command::new("git")
        .current_dir(bare)
        .args(["fsck", "--no-progress", "--connectivity-only", "--no-dangling"])
        .env("GIT_CONFIG_GLOBAL", "/dev/null")
        .output()?;
    ...
    if !fsck.status.success() {
        return Ok(fail(
            fixture,
            "object_creation",
            format!("git fsck rejected: {}", String::from_utf8_lossy(&fsck.stderr).trim()),
        ));
    }
...
fn check_repository_format_readback(
    ...
    if f.algorithm.name() != stock_object.trim() {
        return Ok(fail(
            fixture,
            "format_readback",
            format!("algorithm {} want {}", f.algorithm.name(), stock_object.trim()),
        ));
    }
    Ok(ok(
        fixture,
