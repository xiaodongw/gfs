...
fn pass(check: &'static str, detail: impl Into<String>) -> CheckResult {
    CheckResult { check, outcome: Outcome::Pass, detail: detail.into() }
}
fn fail(check: &'static str, detail: impl Into<String>) -> CheckResult {
    CheckResult { check, outcome: Outcome::Fail, detail: detail.into() }
}
fn reject(check: &'static str, detail: impl Into<String>) -> CheckResult {
    CheckResult { check, outcome: Outcome::ExpectedReject, detail: detail.into() }
}
fn finding(check: &'static str, detail: impl Into<String>) -> CheckResult {
    CheckResult { check, outcome: Outcome::Finding, detail: detail.into() }
}
...
fn run_checks(root: &Path, repo_name: &str) -> Result<Vec<CheckResult>> {
    ...
    anyhow::ensure!(bare.join("objects").is_dir(), "{} is not bare", bare.display());
...
fn check_advertisement_framing(bare: &Path) -> Result<Vec<CheckResult>> {
    ...
    if v0.starts_with(&want) {
        ...
        out.push(fail(
            "advert_v0_preamble",
            format!("first bytes: {:?}", String::from_utf8_lossy(&v0[..32.min(v0.len())])),
        ));
    }
    ...
    let first = lines.first().map(|l| String::from_utf8_lossy(l).trim().to_string());
    if first.as_deref() == Some("version 2") && !v2.starts_with(&preamble) {
        ...
        out.push(fail("advert_v2_no_preamble", format!("first pkt-line was {first:?}")));
    }

    // The same repository with filtering disabled must not advertise it.
    let disabled = UploadPack::new(
        bare,
        UploadPackPolicy { filter: FilterPolicy::Disabled, ..Default::default() },
    )?;
...
fn check_http_headers(gw: &RunningGateway, repo: &str) -> Result<CheckResult> {
    ...
    if ct && cache {
        Ok(pass("http_headers", "advertisement content type and no-cache present"))
    } else {
        Ok(fail("http_headers", format!("content_type={ct} no_cache={cache}")))
    }
}
...
fn check_hidden_refs(bare: &Path, url: &str, work: &Path) -> Result<Vec<CheckResult>> {
    ...
    let head_tree = git_ok(bare, &["rev-parse", "HEAD^{tree}"])?.trim().to_string();
    let orphan = git_ok(bare, &["commit-tree", &head_tree, "-p", &head, "-m", "probe orphan"])
    ...
    match leaked {
        Some(label) => out.push(fail("hidden_refs", format!("refs/xvfs/ advertised over {label}"))),
        None => out.push(pass("hidden_refs", "refs/xvfs/ absent from v0 and v2 advertisements")),
    }

    let leaked_versions: Vec<&str> =
        per_version.iter().filter(|(_, l)| *l).map(|(v, _)| *v).collect();
    if per_version.is_empty() {
        out.push(fail("unadvertised_want", "could not construct an unadvertised commit"));
...
fn check_clone_matrix(bare: &Path, url: &str, work: &Path) -> Result<Vec<CheckResult>> {
    ...
    git_ok(work, &["clone", "-q", "--bare", &bare.to_string_lossy(), &oracle.to_string_lossy()])?;
    let oracle_head = git_ok(&oracle, &["rev-parse", "HEAD"])?.trim().to_string();
    let oracle_tree = git_ok(&oracle, &["rev-parse", "HEAD^{tree}"])?.trim().to_string();
    ...
    for (name, proto, extra) in [
        ...
        ("clone_blob_none", "protocol.version=2", vec!["--filter=blob:none"]),
    ] {
        ...
        if !res.status.success() {
            out.push(fail(name, String::from_utf8_lossy(&res.stderr).trim().to_string()));
            continue;
        }

        // Independent verification, not "the clone exited zero".
        let fsck = git(&dst, &["fsck", "--no-progress", "--no-dangling", "--connectivity-only"])?;
        let head = git_ok(&dst, &["rev-parse", "HEAD"])?.trim().to_string();
        let tree = git_ok(&dst, &["rev-parse", "HEAD^{tree}"])?.trim().to_string();

        if !fsck.status.success() {
            out.push(fail(
                name,
                format!("git fsck failed: {}", String::from_utf8_lossy(&fsck.stderr).trim()),
            ));
            ...
            out.push(pass(
                name,
                format!("fsck clean; HEAD and tree match direct clone (shallow={shallow} promisor={promisor})"),
            ));
        }
    }
    Ok(out)
...
fn check_filter_policy(url: &str, work: &Path) -> Result<Vec<CheckResult>> {
    ...
    for spec in ["tree:0", "blob:limit=1k", "combine:blob:none+tree:0", "object:type=blob"] {
        let dst = work.join(format!("filter-{}", spec.replace([':', '=', '+'], "-")));
        let _ = std::fs::remove_dir_all(&dst);
        let res = Command::new("git")
            .current_dir(work)
            .args(["-c", "protocol.version=2", "clone", "-q", &format!("--filter={spec}"), url, &dst.to_string_lossy()])
            .env("GIT_CONFIG_GLOBAL", "/dev/null")
            .env("GIT_CONFIG_SYSTEM", "/dev/null")
            .env("GIT_TERMINAL_PROMPT", "0")
            .output()?;
...
fn check_gzip_body(bare: &Path) -> Result<CheckResult> {
    ...
    if up.decompress_body(&gz)? != request {
        return Ok(fail("gzip_body", "decompressed body differs from the original"));
    }
    ...
    Ok(pass(
        "gzip_body",
        format!("{} byte body round-trips; a {ratio}:1 bomb is refused", request.len()),
    ))
}

fn check_repo_selection(root: &Path) -> Result<CheckResult> {
    let gw = Gateway { root: root.to_path_buf(), policy: UploadPackPolicy::default() };
    ...
    Ok(pass(
        "repo_selection",
        format!("{} traversal and absolute-path forms rejected", hostile.len()),
    ))
}
